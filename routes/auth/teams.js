...
async function userIsAgent(userId) {
  const result = await pool.query(
    `SELECT id FROM v_b_users u WHERE u.id = $1 AND ${AGENTS_WHERE}`,
    [userId]
  );
...
router.get("/planning", async (_req, res) => {
  try {
    ...
    const teams = (result.rows || []).map((row) => {
      const memberUserIds = Array.isArray(row.member_user_ids)
    ...
    return res.status(500).json({ error: "Erreur lors du chargement des équipes" });
  }
});
...
router.get("/:teamId", adminOnly, [param("teamId").isUUID()], async (req, res) => {
  ...
  try {
    ...
    const members = await pool.query(
      `SELECT m.*, u.email, u.username, u.profile, u.is_active, p.label AS profile_label
       FROM v_b_team_members m
       JOIN v_b_users u ON u.id = m.user_id
       LEFT JOIN v_b_users_profiles p ON p.name = u.profile
       WHERE m.team_id = $1 AND COALESCE(u.role, '') <> 'client'
       ORDER BY m.is_leader DESC, u.email ASC`,
      [req.params.teamId]
    );
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      await ensureTeamsSchema();
      const result = await pool.query(
        `INSERT INTO v_b_teams (name, description, color, icon, is_active, display_order, created_at, updated_at)
        ...
        [
          ...
          String(req.body.icon || "mdi:account-group-outline").trim(),
          req.body.isActive !== false,
          Number(req.body.displayOrder) || 0,
        ]
      );
      return res.status(201).json(mapTeamRow(result.rows[0], { memberCount: 0, leaderCount: 0 }));
    } catch (err) {
      if (err?.code === "23505") {
        return res.status(409).json({ error: "Une équipe avec ce nom existe déjà" });
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!(await userIsAgent(userId))) {
        return res.status(400).json({ error: "Seuls les comptes agents peuvent rejoindre une équipe" });
      }

      const result = await pool.query(
        `INSERT INTO v_b_team_members (team_id, user_id, is_leader)
         VALUES ($1, $2, $3)
         RETURNING *`,
        [req.params.teamId, userId, req.body.isLeader === true]
      );
      ...
      if (err?.code === "23505") {
        return res.status(409).json({ error: "Cet agent est déjà membre de l'équipe" });
      }
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      await ensureTeamsSchema();
      const result = await pool.query(
        `UPDATE v_b_team_members
         SET is_leader = $3
         WHERE team_id = $1 AND user_id = $2
         RETURNING *`,
        [req.params.teamId, req.params.userId, req.body.isLeader === true]
      );
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      await ensureTeamsSchema();
      const result = await pool.query(
        `DELETE FROM v_b_team_members WHERE team_id = $1 AND user_id = $2`,
