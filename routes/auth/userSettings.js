...
import { USER_AVATAR_SETTING_KEY, validatePresetAvatarId, upsertUserAvatarSetting } from '../../utils/userAvatar.js';
...
router.get('/:key', verifyJWT, async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT setting_value 
       ...
       WHERE user_id = $1 AND setting_key = $2`, [userId, key]);
  ...
  try {
    ...
    if (key === USER_AVATAR_SETTING_KEY) {
      ...
      if (value?.type === "preset" || presetId) {
        ...
        await upsertUserAvatarSetting(userId, {
          type: "preset",
          presetId
        });
        return res.json({
      ...
      return res.status(400).json({
        error: "Use /api/users/me/avatar for uploads."
      });
    }
...
router.get('/', verifyJWT, async (req, res) => {
  try {
    const userId = req.user.id;
    const result = await pool.query(`SELECT setting_key, setting_value 
       FROM v_b_users_settings 
