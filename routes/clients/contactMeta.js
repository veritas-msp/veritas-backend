...
export function registerContactMetaRoutes(router, { invalidateContactsListCache } = {}) {
  router.get("/:id/tags", verifyJWT, async (req, res) => {
    try {
      ...
      const existing = await pool.query(
        "SELECT id FROM v_b_contacts WHERE id = $1 LIMIT 1",
        [contactId]
      );
      ...
      res.status(500).json({ error: "Erreur lors du chargement des étiquettes du contact" });
    }
  });
  ...
  router.post("/:id/tags", verifyJWT, async (req, res) => {
    try {
      ...
      const existing = await pool.query(
        "SELECT id, client_id FROM v_b_contacts WHERE id = $1 LIMIT 1",
        [contactId]
      );
      if (existing.rows.length === 0) {
        return res.status(404).json({ error: "Contact introuvable" });
      ...
      await pool.query(
        `INSERT INTO v_b_contact_tag_links (contact_id, tag_id, created_at)
         VALUES ($1, $2, NOW())
      ...
      if (isMissingTableError(err)) {
        return res.status(503).json({
          error: "Les étiquettes contact ne sont pas encore installées (migration requise)",
        });
      }
  ...
  router.delete("/:id/tags/:tagId", verifyJWT, async (req, res) => {
    try {
      ...
      await pool.query(
        "DELETE FROM v_b_contact_tag_links WHERE contact_id = $1 AND tag_id = $2",
        [contactId, req.params.tagId]
      );
