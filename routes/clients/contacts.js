...
import {
  ...
  resetPortalPassword,
  getPortalUserByContactId,
} from '../../utils/contactPortal.js';
import {
  registerContactMetaRoutes,
  fetchTagsByContactIdMap,
  attachContactTags,
  fetchTagsForContactId,
} from './contactMeta.js';
...
import {
  ...
  assertCommunityContactsLimit,
  getActiveClientPortalCount,
...
const buildContactChanges = (oldData, newData) => {
  const fields = [
    ...
    { key: 'sexe', label: 'civilité' },
    { key: 'email', label: 'email' },
    { key: 'telephone', label: 'téléphone' },
    { key: 'communications', label: 'moyens de communication' },
    { key: 'poste', label: 'poste' },
    { key: 'statut', label: 'statut' },
    { key: 'client_id', label: 'entreprise' },
  ];
  ...
  fields.forEach(({ key, label }) => {
    ...
    const oldSerialized =
      key === 'communications' ? JSON.stringify(oldValue ?? []) : String(oldValue ?? '');
    const newSerialized =
      key === 'communications' ? JSON.stringify(newValue ?? []) : String(newValue ?? '');
...
function requireAgent(req, res, next) {
  if (String(req.user?.role || "").toLowerCase() === "client") {
    return res.status(403).json({ error: "Accès réservé aux agents MSP." });
  }
  next();
}
...
async function loadContactById(contactId) {
  const { rows } = await pool.query(
    `SELECT cts.id, cts.client_id, cli.name AS client_name,
...
function resolveContactCommunications(payload, current = null) {
  ...
  return syncLegacyContactFields({
    ...
    email: Object.prototype.hasOwnProperty.call(payload, 'email')
      ? payload.email
      : current?.email,
    telephone: Object.prototype.hasOwnProperty.call(payload, 'telephone')
      ? payload.telephone
      : current?.telephone,
  });
}
  ...
  try {
    const result = await pool.query(
      `SELECT ${CONTACT_LIST_SELECT}
       ${CONTACT_LIST_FROM}
       ${whereClause}
       ORDER BY cts.nom NULLS LAST, cts.prenom NULLS LAST, cts.id`
      , params
    );
...
router.get('/portal/usage', verifyJWT, requireAgent, async (_req, res) => {
  try {
    if (!isCommunity()) {
      return res.json({ limited: false, active: 0, max: null });
    }
...
router.post('/:id/portal', verifyJWT, requireAgent, requirePermission('contacts.manage'), async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ error: err.message || "Impossible de créer le compte portail." });
  }
});
...
router.patch('/:id/portal', verifyJWT, requireAgent, requirePermission('contacts.manage'), async (req, res) => {
  ...
  try {
    if (req.body?.is_active === true) {
      ...
      if (!contactBefore) return res.status(404).json({ error: "Contact non trouvé" });
    ...
    res.status(400).json({ error: err.message || "Mise à jour impossible." });
  }
});
...
router.delete('/:id/portal', verifyJWT, requireAgent, requirePermission('contacts.manage'), async (req, res) => {
  ...
  try {
    ...
    if (!removed) return res.status(404).json({ error: "Aucun compte portail pour ce contact." });
    invalidateContactsListCache(contact?.client_id);
    invalidateContactsListCache(null);
    res.json({ success: true });
  } catch (err) {
...
router.post('/:id/portal/impersonate', verifyJWT, requireRole('admin'), async (req, res) => {
  ...
  if (req.cookies?.[IMPERSONATOR_COOKIE]) {
    return res.status(400).json({
      error: "Une impersonation est déjà active. Quittez-la avant d'en démarrer une nouvelle.",
    });
  }
  ...
  try {
    ...
    if (!contact.portal_user_id) {
      return res.status(404).json({ error: "Aucun compte portail pour ce contact." });
    }
    if (!contact.portal_active) {
      return res.status(403).json({ error: "Le compte portail est désactivé." });
    }

    const portalUser = await getPortalUserByContactId(contactId);
    if (!portalUser || portalUser.id !== contact.portal_user_id) {
      return res.status(404).json({ error: "Compte portail introuvable." });
    }
    if (portalUser.is_active === false) {
      return res.status(403).json({ error: "Le compte portail est désactivé." });
    }
    if (!portalUser.client_id) {
      return res.status(400).json({ error: "Ce contact n'est rattaché à aucune entreprise." });
    }

    const agentToken = req.cookies?.token;
    if (!agentToken) {
      return res.status(401).json({ error: "Session agent introuvable." });
    }

    const clientToken = signSessionToken(
      buildImpersonationClientPayload(portalUser, {
        id: req.user.id,
...
router.post('/', verifyJWT, requirePermission('contacts.create'), async (req, res) => {
  try {
    ...
    const syncedComms = resolveContactCommunications({ communications, email, telephone });
    ...
    `, [
      ...
      normalizedSexe,
      syncedComms.email,
      syncedComms.telephone,
      poste || null,
      statut || 'actif',
      client_id || null,
      JSON.stringify(syncedComms.communications),
    ]);
    ...
    await dispatchNotificationEvent({
      ...
      context: {
        contact: newContact,
        entreprise: { id: String(newContact.client_id || "") },
      },
    }).catch(() => {});
...
router.put('/:id', verifyJWT, requirePermission('contacts.edit'), async (req, res) => {
  try {
    ...
    const { nom, prenom, sexe, email, telephone, poste, statut, client_id, communications } = payload;

    const existing = await pool.query(
      `SELECT id, client_id, nom, prenom, sexe, email, telephone, poste, statut,
    ...
    const resolvedSexe = hasOwn('sexe')
      ? normalizeContactSexe(sexe)
      : (current.sexe || null);
    ...
    const { modifiedFields, changes } = buildContactChanges(current, updatedContact);
    ...
    if (modifiedFields.length > 0) {
      try {
        await pool.query(
          `INSERT INTO v_b_contacts_logs
           (contact_id, client_id, user_id, action, details, created_at)
           VALUES ($1, $2, $3, $4, $5, NOW())`,
          [
            updatedContact.id,
            updatedContact.client_id,
            userId,
            'Modification du contact',
            JSON.stringify({ modifiedFields, changes })
          ]
        );
      } catch (logError) {
    ...
    await dispatchNotificationEvent({
      ...
      context: {
        ...
        changes,
        entreprise: { id: String(updatedContact.client_id || "") },
      },
    }).catch(() => {});
...
router.delete('/:id/logs', verifyJWT, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: `${result.rows.length} logs supprimés` });
  } catch (err) {
    res.status(500).json({ error: "Erreur lors de la purge des logs", details: err.message });
  }
});
...
router.delete('/:id', verifyJWT, requirePermission('contacts.delete'), async (req, res) => {
  try {
    const { id } = req.params;
    const existing = await pool.query(
      `SELECT client_id FROM v_b_contacts WHERE id = $1`,
      [parseInt(id)]
    );
