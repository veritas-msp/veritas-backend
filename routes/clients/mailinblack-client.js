...
router.post('/test-credentials', async (req, res) => {
  try {
    const { apiUrl, apiKey, authKey, authClientId } = req.body;
    const key = (authKey || apiKey || '').trim();
    if (!apiUrl || !key) {
      return res.status(400).json({
        success: false,
        error: 'URL API et clé API sont requis',
      });
    }
    const credentials = {
      apiUrl: apiUrl.trim(),
    ...
    res.json({
      success: true,
      message: 'Connexion Mailinblack Protect réussie',
    ...
    res.status(status >= 400 && status < 600 ? status : 500).json({
      success: false,
      error: err.message || 'Échec du test de connexion',
    });
  }
});
...
router.post('/:clientId', async (req, res) => {
  try {
    const { clientId } = req.params;
    const { label, apiUrl, apiKey, authKey, authClientId, solution } = req.body;
    const key = (authKey || apiKey || '').trim();
    ...
    const result = await pool.query(
      `INSERT INTO v_b_clients_mailinblack (client_id, label, solution, api_url, api_key_encrypted, iv, auth_tag, auth_client_id, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, CURRENT_TIMESTAMP)
       RETURNING id, client_id, label, solution, api_url, auth_client_id, created_at, updated_at`,
      [
        clientId,
        label || `Tenant Mailinblack ${clientId}`,
...
router.put('/:clientId/:tenantId', async (req, res) => {
  try {
    ...
    const existing = await pool.query(
      `SELECT id FROM v_b_clients_mailinblack WHERE id = $1 AND client_id = $2`,
      [tenantId, clientId]
    );
    ...
    if (incomingKey) {
      const { encrypted, iv, authTag } = encrypt(incomingKey);
    ...
    const result = await pool.query(
      `UPDATE v_b_clients_mailinblack
       SET ${fields.join(', ')}
...
router.delete('/:clientId/:tenantId', async (req, res) => {
  try {
    const { clientId, tenantId } = req.params;
    const result = await pool.query(
      `DELETE FROM v_b_clients_mailinblack WHERE id = $1 AND client_id = $2 RETURNING id`,
      [tenantId, clientId]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, error: 'Tenant introuvable' });
...
router.post('/:clientId/:tenantId/test', async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Connexion Mailinblack Protect réussie' });
  } catch (err) {
    const status = err.status || 500;
    res.status(status >= 400 && status < 600 ? status : 500).json({
      success: false,
