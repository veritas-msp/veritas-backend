...
router.get("/", async (req, res) => {
  try {
    ...
    const modules = await listContractModuleOptions({ includeDisabled });
    res.json({ modules });
  } catch (err) {
...
router.get("/admin", verifyJWT, requireRole("admin"), async (_req, res) => {
  try {
    const modules = await listContractModuleOptions({ includeDisabled: true, includeUsage: true });
    res.json({ modules });
  } catch (err) {
...
router.post(
  ...
  [
    ...
    body("moduleKey").optional().isString().trim(),
