...
import {
  ...
  SLA_SETTINGS_KEY,
  SLA_SETTINGS_SECTION,
  normalizeSlaSettings,
} from "../../utils/slaSettings.js";
...
async function readSlaSettingsFromDb() {
  try {
    ...
    const result = await pool.query(
      ...
      [SLA_SETTINGS_SECTION, SLA_SETTINGS_KEY]
    );
    ...
    try {
      ...
      return normalizeSlaSettings({ ...DEFAULT_SLA_SETTINGS, ...parsed });
    } catch {
      return normalizeSlaSettings(DEFAULT_SLA_SETTINGS);
    }
  } catch (err) {
...
async function upsertSlaSettings(client, settings) {
  await client.query(
    ...
    [SLA_SETTINGS_KEY, JSON.stringify(settings), "Paramètres SLA support", SLA_SETTINGS_SECTION]
  );
}
...
router.get("/", verifyJWT, async (_req, res) => {
  try {
    ...
    res.status(500).json({ error: "Impossible de charger les paramètres SLA." });
  }
});

router.patch("/", verifyJWT, requireRole("admin"), async (req, res) => {
  const existing = await readSlaSettingsFromDb();
  const normalized = normalizeSlaSettings({ ...existing, ...req.body });
  ...
  try {
    ...
    res.status(500).json({ error: "Impossible d'enregistrer les paramètres SLA." });
  } finally {
    client.release();
  }
});
