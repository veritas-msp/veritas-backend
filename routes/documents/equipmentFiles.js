...
const ALLOWED_MIME = new Set(["image/jpeg", "image/png", "image/gif", "image/webp", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/plain", "text/csv"]);
...
router.get("/", verifyJWT, requirePermission("documents.view"), async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT id, client_id, equipment_id, equipment_type, equipment_name,
  ...
  try {
    ...
    if (!clientId || !equipmentId) {
      ...
      return res.status(400).json({
        error: "clientId et equipmentId required."
      });
    }
    if (!isUuid(equipmentId)) {
      fs.unlinkSync(req.file.path);
      return res.status(400).json({
        error: "Invalid equipment identifier."
      });
    }
    const result = await pool.query(`INSERT INTO v_b_equipment_files
       ...
       RETURNING id, client_id, equipment_id, equipment_type, equipment_name, file_name,
                 mime_type, size_bytes, category, description, created_at`, [Number(clientId), String(equipmentId), equipmentType || null, equipmentName || null, req.file.originalname, req.file.filename, req.file.mimetype, req.file.size, category, description, resolveFileUploadedBy(req.user)]);
    ...
    const message = err.message?.includes("bigint") && err.message?.includes("uuid") ? "Database schema error (uploaded_by). Restart the server to apply migrations." : err.message || "Error during upload.";
    res.status(500).json({
      error: message
    });
  }
...
router.get("/:id/download", verifyJWT, requirePermission("documents.view"), async (req, res) => {
  try {
    const result = await pool.query(`SELECT file_path, file_name, mime_type FROM v_b_equipment_files WHERE id = $1 AND is_deleted = FALSE`, [req.params.id]);
...
router.delete("/:id", verifyJWT, requirePermission("documents.delete"), async (req, res) => {
  try {
    const result = await pool.query(`UPDATE v_b_equipment_files SET is_deleted = TRUE, updated_at = NOW()
