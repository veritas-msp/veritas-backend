...
router.get("/tags/batch", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT l.equipment_id::text AS equipment_id,
       ...
       ORDER BY l.equipment_id, t.label ASC`,
      [clientIds]
    );
    ...
    res.status(500).json({ error: "Erreur lors du chargement des étiquettes périphériques" });
  }
});
...
router.get("/:equipmentId/activity", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Erreur lors du chargement de l'activité du périphérique" });
  }
});
...
router.get("/:equipmentId/tags", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Erreur lors du chargement des étiquettes du périphérique" });
  }
});
...
router.post("/:equipmentId/tags", async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO v_b_equipment_tag_links (equipment_id, client_id, tag_id, created_at)
       VALUES ($1, $2, $3, NOW())
       ON CONFLICT (equipment_id, tag_id) DO NOTHING`,
      [equipmentId, clientId, tagResult.rows[0].id]
    );
    ...
    if (isMissingTableError(err)) {
      return res.status(503).json({
        error: "Les étiquettes périphérique ne sont pas encore installées (migration requise)",
      });
    }
...
router.delete("/:equipmentId/tags/:tagId", async (req, res) => {
  try {
    ...
    await pool.query(
      "DELETE FROM v_b_equipment_tag_links WHERE equipment_id = $1 AND client_id = $2 AND tag_id = $3",
      [equipmentId, clientId, req.params.tagId]
    );
