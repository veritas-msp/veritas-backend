...
const SYNC_MIN_INTERVAL_MS = 30 * 60 * 1000; // 30 minutes
...
function parseEventStateRaw(rawState) {
  ...
  if (typeof rawState === 'string') {
    const match =
      ...
      rawState.match(/\b(OK|WARNING|CRITICAL|UNKNOWN)\b/i);
    if (match) {
...
export function computeMonitoringSummary(monitoringData, lastSyncedAt) {
  ...
  let status = 'ok';
  if (critServices > 0 || recentCritAlerts > 0) status = 'critical';
  else if (warnServices > 0 || recentWarnAlerts > 0) status = 'warning';
...
async function getStoredMonitoring(equipmentId) {
  const r = await pool.query(
    `SELECT * FROM ${TABLE} WHERE equipment_id = $1::uuid`,
    [equipmentId]
  );
...
async function verifyEquipmentMapping(equipmentId, clientId, family, hostName) {
  ...
  const r = await pool.query(
    `SELECT id, checkmk_host_name FROM ${table}
     WHERE id = $1::uuid AND client_id = $2 AND checkmk_host_name = $3`,
...
async function fetchAndMergeCheckMKData(req, { hostName, site, existingMonitoringData, incrementalFrom }) {
  ...
  const eventsStartTime = incrementalFrom
    ? new Date(new Date(incrementalFrom).getTime() - 24 * 60 * 60 * 1000)
    : (() => {
  ...
  const queryBase = { site: siteParam };

  const [services, events, hostDetails, hostEventsDetailed, avail1m, avail3m, avail1y] = await Promise.all([
    internalCheckMKGet(req, `/services/${encodeURIComponent(hostName)}`, {
      ...queryBase,
    ...
    internalCheckMKGet(req, `/host/${encodeURIComponent(hostName)}`, queryBase),
    internalCheckMKGet(req, `/host-events/${encodeURIComponent(hostName)}`, {
      ...queryBase,
      start_time: eventsStartTime.toISOString(),
      end_time: eventsEndTime.toISOString(),
    }),
    (async () => {
      const { startTime, endTime } = getAvailabilityPeriodRange('1m');
    ...
    (async () => {
      const { startTime, endTime } = getAvailabilityPeriodRange('3m');
      const data = await internalCheckMKGet(req, `/availability-table/${encodeURIComponent(hostName)}`, {
        ...queryBase,
        start_time: startTime.toISOString(),
        end_time: endTime.toISOString(),
    ...
    (async () => {
      const { startTime, endTime } = getAvailabilityPeriodRange('1y');
      const data = await internalCheckMKGet(req, `/availability-table/${encodeURIComponent(hostName)}`, {
        ...queryBase,
        start_time: startTime.toISOString(),
        end_time: endTime.toISOString(),
  ...
  const availabilityByPeriod = {
    ...
    ...(avail1m != null ? { '1m': avail1m } : {}),
    ...(avail3m != null ? { '3m': avail3m } : {}),
    ...(avail1y != null ? { '1y': avail1y } : {}),
  };
...
export async function runEquipmentMonitoringSync(req, {
  ...
  if (!equipmentId || !clientId || !family || !hostName) {
    throw new Error('Paramètres manquants: equipmentId, clientId, family, hostName sont requis.');
  }

  const isMapped = await verifyEquipmentMapping(equipmentId, clientId, family, hostName);
  if (!isMapped) {
    throw new Error('Équipement non trouvé ou non mappé à ce host CheckMK.');
  }
  ...
  if (!force && existing?.last_synced_at) {
    ...
    if (!Number.isNaN(lastSyncMs) && Date.now() - lastSyncMs < SYNC_MIN_INTERVAL_MS) {
      return {
        ...
        skipped: true,
        message: 'Synchronisation récente (< 30 min), données en base utilisées.',
      };
    }
  }

  const { monitoringData, hostDetails } = await fetchAndMergeCheckMKData(req, {
    hostName,
    site,
  ...
  if (existing) {
    await pool.query(
      `UPDATE ${TABLE}
       SET monitoring_data = $1::jsonb,
           ...
           updated_at = NOW()
       WHERE equipment_id = $6::uuid`,
      [
        JSON.stringify(monitoringData),
        hostDetails ? JSON.stringify(hostDetails) : null,
        hostName,
        site || null,
        nowIso,
        equipmentId,
    ...
    await pool.query(
      `INSERT INTO ${TABLE}
       ...
       VALUES ($1::uuid, $2, $3, $4, $5, $6::jsonb, $7::jsonb, $8::timestamptz)`,
      [
        equipmentId,
        clientId,
        family,
...
router.post('/equipment-monitoring/summaries', verifyJWT, async (req, res) => {
  try {
    const { clientId, equipmentIds } = req.body || {};
    let rows = [];

    if (clientId != null) {
      const r = await pool.query(
        `SELECT equipment_id, monitoring_data, last_synced_at FROM ${TABLE} WHERE client_id = $1`,
        [clientId]
      );
      rows = r.rows;
    } else if (Array.isArray(equipmentIds) && equipmentIds.length > 0) {
      const r = await pool.query(
        `SELECT equipment_id, monitoring_data, last_synced_at FROM ${TABLE} WHERE equipment_id = ANY($1::uuid[])`,
        [equipmentIds]
      );
      rows = r.rows;
    } else {
    ...
    res.status(500).json({ error: err.message || 'Erreur lors de la lecture des résumés monitoring.' });
  }
});

// ───────────────────────────────────────────────
// GET /api/checkmk/equipment-monitoring/:equipmentId
// ───────────────────────────────────────────────
router.get('/equipment-monitoring/:equipmentId', verifyJWT, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: err.message || 'Erreur lors de la lecture des données monitoring.' });
  }
});

// ───────────────────────────────────────────────
// POST /api/checkmk/equipment-monitoring/sync
// Body: { equipmentId, clientId, family, hostName, site?, force?, availabilityPeriod? }
// ───────────────────────────────────────────────
router.post('/equipment-monitoring/sync', verifyJWT, async (req, res) => {
  try {
    ...
    if (msg.includes('non trouvé') || msg.includes('non mappé')) {
      return res.status(404).json({ error: msg });
    }
    if (msg.includes('Paramètres manquants')) {
