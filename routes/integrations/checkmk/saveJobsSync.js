...
async function loadLastSyncFromDb() {
  try {
    const r = await pool.query(
      `SELECT value FROM ${SETTINGS_TABLE} WHERE section = $1 AND key = $2`,
      [LAST_SYNC_SECTION, LAST_SYNC_KEY]
    );
...
async function saveLastSyncToDb(isoDate) {
  try {
    const r = await pool.query(
      `UPDATE ${SETTINGS_TABLE} SET value = $1 WHERE section = $2 AND key = $3`,
      [isoDate, LAST_SYNC_SECTION, LAST_SYNC_KEY]
    );
    if (r.rowCount === 0) {
      await pool.query(
        `INSERT INTO ${SETTINGS_TABLE} (section, key, value) VALUES ($1, $2, $3)`,
        [LAST_SYNC_SECTION, LAST_SYNC_KEY, isoDate]
      );
    }
  } catch (err) {
    console.warn('[checkmk save-jobs] Impossible d’enregistrer lastSync en base:', err.message);
  }
}
...
/** Format seconds as "Xh Ymin" or "Ymin" */
...
/**
 * Parse duration from CheckMK perf_data (e.g. "duration=3600" or "backup_duration=125;300;600").
 * Returns string like "2h 15min" or null.
 */
function parseDurationFromPerfData(perfData) {
  if (!perfData || typeof perfData !== 'string') return null;
  const str = String(perfData);
  // Keys often used by backup plugins: duration, backup_duration, last_duration, duration_sec, time
...
/**
 * Parse duration from CheckMK plugin_output.
 * Tries: Creation time / End time, then "Duration: ...", "Backup duration: ...", "Dauer: ...",
 * and pre-formatted "Xh Ymin" / "X min" / "X seconds".
 * Returns string like "2h 15min" or null.
 */
function parseDurationFromPluginOutput(pluginOutput) {
  ...
  // 1) Creation time / End time (Veeam-style, format DD.MM.YYYY HH:mm:ss)
    ...
    try {
      const parseDate = (dateStr) => {
        ...
        if (segs.length === 3 && segs[2] >= 2000 && segs[2] <= 2100) {
          const day = segs[0], month = segs[1], year = segs[2];
  ...
  // 2) "Duration: Xh Ymin" or "Duration: X min" or "Backup duration: ..." / "Dauer: ..."
  ...
  // 3) Already formatted in text: "Xh Ymin" or "X min"
...
/**
 * Convert CheckMK last_check to ISO date string for DB (timestamptz).
 * lastCheck can be Unix seconds (number) or ISO string.
 */
...
export async function runSaveJobsSync({ clientId } = {}) {
  ...
  if (!hasLastBackupDate || !hasLastBackupDuration) {
    throw new Error('Colonnes last_backup_date / last_backup_duration absentes. Exécutez la migration add_last_backup_to_save.sql.');
  }
  ...
  const jobsResult = await pool.query(
    `SELECT id, client_id, item_key, data, checkmk_host_name, checkmk_site, checkmk_service_name
     FROM ${SAVE_TABLE}
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      let lastBackupDuration =
        parseDurationFromPerfData(svc.performanceData) ||
        parseDurationFromPerfData(svc.pluginOutput) ||
        parseDurationFromPluginOutput(svc.pluginOutput) ||
        parseDurationFromPluginOutput(svc.longPluginOutput);
      ...
      if (!lastBackupDuration) {
        const serviceNameForView = (serviceName || '').trim().startsWith(`${hostName}:`)
          ...
          : (serviceName || '').trim();
        viewPyData = await getServicePluginOutputViaViewPy(
          settings.apiUrl,
          authData.auth_header,
          hostName,
          serviceNameForView || serviceName,
          site
        );
        if (viewPyData) {
          ...
          lastBackupDuration =
            parseDurationFromPerfData(viewPyData.performanceData) ||
            parseDurationFromPerfData(out) ||
            parseDurationFromPluginOutput(viewPyData.longPluginOutput) ||
            parseDurationFromPluginOutput(viewPyData.pluginOutput);
        }
      }
      if (!lastBackupDuration) {
      ...
      if (!pluginOutputForCreation && hasLastBackupStart) {
        const serviceNameForView = (serviceName || '').trim().startsWith(`${hostName}:`)
          ? (serviceName || '').trim().substring(hostName.length + 1).trim()
          : (serviceName || '').trim();
        const viewPyForStart = await getServicePluginOutputViaViewPy(
          settings.apiUrl,
          authData.auth_header,
          hostName,
          serviceNameForView || serviceName,
      ...
      if (hasLastBackupStart) {
        await pool.query(
          `UPDATE ${SAVE_TABLE}
           ...
           WHERE id = $3`,
          [lastBackupDate, lastBackupDuration || null, job.id, lastBackupStart || null]
        );
      } else {
        await pool.query(
          `UPDATE ${SAVE_TABLE}
           ...
           WHERE id = $3`,
          [lastBackupDate, lastBackupDuration || null, job.id]
        );
      }
      updated += 1;
    } catch (err) {
  ...
  lastSaveJobsSyncAt = new Date().toISOString();
  await saveLastSyncToDb(lastSaveJobsSyncAt);
  return { message: 'Synchronisation terminée', updated, total: jobs.length, lastSync: lastSaveJobsSyncAt };
}
...
router.get('/save-jobs/last-sync', verifyJWT, async (req, res) => {
  ...
  res.json({ lastSync: lastSaveJobsSyncAt });
});
...
router.post('/save-jobs/sync', verifyJWT, async (req, res) => {
  try {
    ...
    if (err.message?.includes('authentifier')) return res.status(502).json({ error: msg });
