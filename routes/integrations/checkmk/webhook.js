...
router.post("/webhook", async (req, res) => {
  try {
    ...
    if (hostName && clientId && webhook.triggerSync !== false) {
      const mkRow = await pool.query(`SELECT equipment_id, equipment_family
         FROM v_b_equipment_checkmk_monitoring
         WHERE client_id = $1 AND checkmk_host_name = $2
         LIMIT 1`, [Number(clientId), String(hostName)]);
      if (mkRow.rows[0]) {
        await pool.query(`UPDATE v_b_equipment_checkmk_monitoring SET updated_at = NOW() WHERE client_id = $1 AND checkmk_host_name = $2`, [Number(clientId), String(hostName)]);
      }
    }
