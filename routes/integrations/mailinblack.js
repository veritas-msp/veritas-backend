...
import {
  resolveMailinblackCredentials,
  getGlobalMailinblackConfigStatus,
} from '../../utils/mailinblackCredentials.js';
import {
  mailinblackProtectCheck,
  mailinblackListCustomers,
  mailinblackGetCustomer,
  mailinblackBuildDashboard,
  formatMailinblackSyncPayload,
} from '../../utils/mailinblackApi.js';
...
function buildInlineCredentials(req) {
  ...
  const bodyClientId = (
    req.body?.MAILINBLACK_CLIENT_ID ||
    req.body?.authClientId ||
    ''
  ).trim();
  if (bodyUrl && bodyKey) {
    return {
...
async function getCredentialsFromRequest(req) {
  ...
  const mailinblackTenantId =
    req.query.mailinblackTenantId || req.body?.mailinblackTenantId || null;
  return resolveMailinblackCredentials({ clientId, mailinblackTenantId });
}
...
router.post('/test', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Connexion à l\'API Mailinblack Protect réussie',
      tenant: {
    ...
    let errorMessage = 'Erreur de connexion à l\'API Mailinblack';
    if (err.status === 401 || /unauthorized|invalid.*key|token/i.test(err.message)) {
      ...
      errorMessage = 'Accès refusé — vérifiez les droits de la clé API';
    } else if (err.message) {
...
router.get('/customers', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: err.message || 'Erreur lors de la récupération des clients Mailinblack',
    });
  }
});
...
router.get('/dashboard/:customerId', async (req, res) => {
  try {
    ...
    const credentials = await getCredentialsFromRequest(req);
    const dashboard = await mailinblackBuildDashboard(
      credentials.apiUrl,
      credentials,
      customerId
    );
    ...
    res.status(500).json({
      success: false,
      error: err.message || 'Erreur lors du chargement du dashboard Mailinblack',
    });
  }
});
...
router.post('/sync/:customerId', async (req, res) => {
  try {
    ...
    const [customer, dashboard] = await Promise.all([
      ...
      mailinblackBuildDashboard(credentials.apiUrl, credentials, customerId).catch(() => null),
    ]);
    if (!customer) {
      return res.status(404).json({ success: false, error: 'Client Mailinblack introuvable' });
    }

    const mappingMode = credentials.source === 'dedicated' ? 'dedicated' : 'reseller';
    const payload = formatMailinblackSyncPayload(
      customer,
      mappingMode,
      credentials.mailinblackTenantId,
      { dashboard }
    );
    ...
    res.status(500).json({
      success: false,
      error: err.message || 'Erreur lors de la synchronisation Mailinblack',
    });
  }
});
