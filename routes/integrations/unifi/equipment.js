...
router.post("/equipment-test", verifyJWT, async (req, res) => {
  try {
    ...
    if (!response.ok) {
      let errorMessage = "Connection to l'API UniFi local failed";
      if (response.status === 401) errorMessage = "Invalid or expired API key";else if (response.status === 403) errorMessage = "Access denied — check API key permissions";else if (response.status === 404) {
        errorMessage = "Endpoint not found — check controller URL (UDM Pro / UniFi OS)";
      }
      return res.status(400).json({
        success: false,
        error: errorMessage,
    ...
    return res.json({
      success: true,
      message: "UniFi API connection successful",
      controller: {
        host: baseUrl,
    ...
    return res.status(500).json({
      success: false,
      error: "Error during tis de connection UniFi",
      details: err.message
    });
  }
});
