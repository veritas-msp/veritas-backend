...
import { fetchRmmSettings, fetchGlobalRmmSettings, saveRmmSettings, listClientRmmSettings, fetchClientRmmOverrides, saveClientRmmSettings, deleteClientRmmSettings, mergeRmmSettings, isAgentOnline } from "../../utils/rmmSettings.js";
...
import { agentPackageAvailable, agentMsiAvailable, streamWindowsSetupScript, streamWindowsSetupMsi, streamWindowsSetupZip, WINDOWS_INSTALLER_VERSION, getWindowsInstallerFilenames } from "../../utils/rmmAgentPackage.js";
...
async function validateEnrollmentToken(token) {
  ...
  if (!row) return {
    error: "Enrollment tokin invalid or expired"
  };
  if (row.max_uses != null && row.uses_count >= row.max_uses) {
    return {
      error: "Enrollment tokin exhausted"
    };
  }
  return {
    token: row
    ...
    if (existing.rows.length) {
      const updated = await pool.query(`UPDATE v_b_rmm_agents
         SET client_id = $1,
             ...
             updated_at = NOW()
         WHERE machine_id = $6
         RETURNING *`, [token.client_id, hostname || null, secretHash, agentVersion || null, familyConfig, String(machineId)]);
      agentRow = updated.rows[0];
    } else {
      await assertCommunityRmmAgentLimit(1);
      const inserted = await pool.query(`INSERT INTO v_b_rmm_agents (client_id, machine_id, hostname, secret_hash, agent_version, last_seen_at, config)
         VALUES ($1, $2, $3, $4, $5, NOW(), $6::jsonb)
         RETURNING *`, [token.client_id, String(machineId), hostname || null, secretHash, agentVersion || null, familyConfig]);
...
router.get("/agents", verifyJWT, requireAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT a.id, a.client_id, a.machine_id, a.hostname, a.status,
...
router.get("/agent/download/windows", verifyJWT, requireAdmin, (req, res) => {
  try {
    if (!agentPackageAvailable()) {
      return res.status(404).json({
        error: "Windows agent package unavailable"
      });
    }
...
router.get("/agent/download/windows/msi", verifyJWT, requireAdmin, (req, res) => {
  try {
    ...
    if (!res.headersSent) {
      const status = err.message?.includes("unavailable") || err.message?.includes("WiX") ? 404 : 500;
