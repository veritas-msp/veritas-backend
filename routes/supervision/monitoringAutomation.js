...
import { getMonitoringAutomationConfig, saveMonitoringAutomationConfig, getMonitoringRunbooks, saveMonitoringRunbooks, DEFAULT_MONITORING_AUTOMATION_CONFIG, DEFAULT_MONITORING_RUNBOOKS } from "../../utils/monitoringAutomationConfig.js";
import { enableMonitoringAlertsForClient, loadSupervisionEquipmentInventory } from "../../utils/equipmentInventoryScan.js";
import { computeMonitoringAlertStats, computeMonitoringMetrics } from "../../services/monitoringMetricsService.js";
import { ingestExternalMonitoringEvent, listMonitoringEvents } from "../../services/monitoringEventQueue.js";
...
router.get("/config", verifyJWT, requirePermission("supervision.view"), async (req, res) => {
  try {
    ...
    res.json({
      config,
      defaults: DEFAULT_MONITORING_AUTOMATION_CONFIG
    });
  } catch (err) {
...
router.get("/runbooks", verifyJWT, requirePermission("supervision.view"), async (req, res) => {
  try {
    ...
    res.json({
      runbooks,
      defaults: DEFAULT_MONITORING_RUNBOOKS
    });
  } catch (err) {
...
router.post("/clients/:clientId/enable-alerts", verifyJWT, requirePermission("supervision.manage"), async (req, res) => {
  try {
...
router.get("/clients/:clientId/inventory-summary", verifyJWT, requirePermission("supervision.view"), async (req, res) => {
  try {
    const clientId = Number(req.params.clientId);
    if (!clientId) return res.status(400).json({
      error: "Invalid clientId"
...
router.get("/alert-stats", verifyJWT, requirePermission("supervision.view"), async (req, res) => {
  try {
    const clientId = req.query.clientId ? Number(req.query.clientId) : null;
    const stats = await computeMonitoringAlertStats({
      clientId
    });
    res.json(stats);
  } catch (err) {
...
router.get("/equipment/:equipmentId/timeline", verifyJWT, requirePermission("supervision.view"), async (req, res) => {
  try {
