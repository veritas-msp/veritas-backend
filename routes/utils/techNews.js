...
import {
  ...
  addReactionStreamClient,
  broadcastReactionHeartbeat,
  ensureTechNewsReactionsTable,
  getReactionsForArticles,
  removeReactionStreamClient,
  toggleTechNewsReaction,
} from "../../utils/techNewsReactions.js";
import {
  ALLOWED_FEED_CATEGORIES,
  createTechNewsFeed,
  deleteTechNewsFeed,
  getTechNewsFeedsMeta,
  listTechNewsFeeds,
  resetTechNewsFeedsForLocale,
  updateTechNewsFeed,
} from "../../utils/techNewsFeedsConfig.js";
...
router.post(
  ...
  [
    ...
    body("category").optional().isIn(ALLOWED_FEED_CATEGORIES),
...
router.post(
  ...
  verifyJWT,
  [
    body("articleId").isString().trim().notEmpty(),
    body("emoji").isString().trim().isIn(ALLOWED_REACTION_EMOJIS),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    try {
      const result = await toggleTechNewsReaction(
        req.body.articleId,
        req.user.id,
        req.body.emoji
      );
      ...
      res.status(err.status || 500).json({ error: err.message || "Erreur réaction." });
    }
  }
);
...
router.get("/reactions/stream", verifyJWT, async (req, res) => {
  try {
    ...
    if (!res.headersSent) {
      res.status(500).json({ error: "Flux temps réel indisponible." });
    }
  }
});
