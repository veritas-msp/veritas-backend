...
import {
  loadAssignmentsByViewIds,
  syncViewAssignments,
  getUserTeamIds,
  isAssignedVisibility,
  hasAssignmentTargets,
  userCanAccessAssignedView,
  mapViewAssignments,
} from "../../services/ticketViewAssignments.js";
...
async function resolveViewRulesForCount(viewId, customViews = []) {
  ...
  if (
    normalizedId === "__satisfaction_mine__"
    || normalizedId === "__satisfaction_all__"
  ) {
    return null;
  }
...
router.get(
  ...
  verifyJWT,
  [
    query("pageScope").optional().isIn(["ticket", "ticket_sales"]),
    query("viewId").optional().isString(),
    query("search").optional().isString(),
    query("ticketType").optional().isString(),
    query("scope").optional().isIn(["all", "views", "statuses"]),
  ],
  async (req, res) => {
    ...
    try {
      ...
      if (pageScope !== "ticket") {
        return res.json({ views: {}, statuses: null });
      }
      ...
      if (scope === "all" || scope === "views") {
        const result = await pool.query(
          ...
          [pageScope, userId, userProfileName]
        );
      ...
      if (scope === "all" || scope === "views") {
        const viewsToCount = [
          ...BUILTIN_TICKET_VIEWS.map((view) => ({ id: view.id, rules: view.rules })),
          ...customViews,
        ];

        const viewCountsResults = await Promise.allSettled(
          viewsToCount.map(async (view) => {
            const rules = normalizeRules(view.rules);
            const viewMode = rules.viewMode === "trash" ? "trash" : "active";
            const total = await countTickets(
              pool,
              { viewRules: rules, viewMode },
              schema
            );
      ...
      if (scope === "all" || scope === "statuses") {
        ...
        if (viewId) {
          ...
          if (rules) {
            const viewMode = rules.viewMode === "trash" ? "trash" : "active";
            statuses = await countTicketsByStatus(
              pool,
              {
                viewRules: rules,
                viewMode,
...
const viewBodyValidators = [
  body("name").optional().isString().trim().isLength({ min: 1, max: 120 }),
  body("description").optional({ nullable: true }).isString(),
  body("pageScope").optional().isIn(["ticket", "ticket_sales"]),
  body("visibility").optional().isIn(["private", "public", "profile", "assigned"]),
  ...
  async (req, res) => {
    ...
    try {
      ...
      if ((visibility === "public" || visibility === "assigned") && !isAdminUser(req)) {
        return res.status(403).json({ error: "Seuls les administrateurs peuvent créer une vue assignée ou publique" });
      }

      const assignments = extractAssignmentsFromBody(req.body);
      if (visibility === "assigned" && !hasAssignmentTargets(assignments)) {
        return res.status(400).json({
          error: "Sélectionnez au moins un utilisateur, profil ou équipe pour une vue assignée",
        });
      }
      ...
      const result = await pool.query(
        ...
        [
          ...
          String(req.body.pageScope || "ticket"),
          visibility,
          req.user?.id,
          String(req.body.icon || "mdi:view-list").trim(),
          req.body.color ? String(req.body.color).trim() : null,
          JSON.stringify(rules),
          String(req.body.sortBy || "updated_at"),
          String(req.body.sortDirection || "desc"),
...
router.put(
  "/:viewId",
  verifyJWT,
  [param("viewId").isUUID(), ...viewBodyValidators],
  async (req, res) => {
    if (validationErrorOrNull(req, res)) return;
    try {
      ...
      if (!canWriteView(view, req)) return res.status(403).json({ error: "Modification non autorisée" });

      const nextVisibility = req.body.visibility !== undefined
        ? normalizeVisibility(req.body.visibility)
        : normalizeVisibility(view.visibility);

      if ((nextVisibility === "public" || nextVisibility === "assigned") && !canChangeViewVisibility(nextVisibility, view, req)) {
        return res.status(403).json({ error: "Seuls les administrateurs peuvent publier une vue" });
      }
      ...
      const result = await pool.query(
        ...
        [
          ...
          req.body.description !== undefined ? (req.body.description ? String(req.body.description).trim() : null) : null,
          req.body.visibility !== undefined ? nextVisibility : null,
          req.body.icon !== undefined ? String(req.body.icon).trim() : null,
          req.body.color !== undefined ? (req.body.color ? String(req.body.color).trim() : null) : null,
          req.body.rules !== undefined ? JSON.stringify(rules) : null,
          req.body.sortBy !== undefined ? String(req.body.sortBy) : null,
          req.body.sortDirection !== undefined ? String(req.body.sortDirection) : null,
          req.body.displayOrder !== undefined ? Number(req.body.displayOrder) : null,
        ]
      );
      ...
      if (nextVisibility === "assigned") {
        ...
        await syncViewAssignments(updated.id, { profileNames: [], userIds: [], teamIds: [] });
      }
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!canWriteView(view, req)) return res.status(403).json({ error: "Suppression non autorisée" });
