...
console.log(`Version installateur: ${WINDOWS_INSTALLER_VERSION}`);
...
if (process.platform !== "win32") {
  console.warn("Compilation .msi ignoree (Windows + WiX requis).");
  process.exit(0);
}

const msiBuild = spawnSync(
  "powershell.exe",
  [
    "-NoProfile",
    "-ExecutionPolicy",
    "Bypass",
    "-File",
    "scripts/build-rmm-windows-msi.ps1",
  ],
  {
    cwd: path.resolve(__dirname, ".."),
    stdio: "inherit",
...
if (msiBuild.status !== 0) {
  console.warn(
    "Compilation MSI ignoree (WiX Toolset requis). ZIP et CMD restent disponibles via l'API."
  );
  process.exit(0);
}

if (!fs.existsSync(msiPath)) {
  console.error(`MSI introuvable apres build: ${msiPath}`);
  process.exit(1);
}
