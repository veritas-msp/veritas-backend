...
export async function listPortalVaultFiles(clientId, { category, search, limit, offset } = {}) {
  ...
  if (trimmedSearch) {
    ...
    conditions.push(
      `(file_name ILIKE $${values.length} OR COALESCE(description, '') ILIKE $${values.length} OR category ILIKE $${values.length})`
    );
  }
...
export async function countPortalVaultFiles(clientId, { category, search } = {}) {
  const ready = await ensureVisibleToClientColumn();
...
export async function getPortalVaultFileRecord(clientId, fileId) {
  ...
  const { rows } = await pool.query(
    `SELECT id, client_id, file_name, file_path, mime_type, size_bytes, category, description, created_at
     FROM v_b_client_files
     WHERE ${conditions.join(" AND ")}
     LIMIT 1`,
    [fileId, Number(clientId)]
  );
  return rows[0] || null;
}
