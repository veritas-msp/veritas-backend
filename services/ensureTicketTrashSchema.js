...
export async function ensureTicketTrashSchema({
  ...
  if (!(await canRunAutoSchemaMigrations())) {
    return {
      ready: false,
      error: "Installation in progress — migrations deferred."
    };
  }
  ...
  try {
    ...
    if (!allowAutoMigrate || migrationAttempted) {
      return {
        ready: false,
        error: lastFailureMessage || "Trash columns are missing. Run schema/patches/20260625_ticket_trash.sql as the table owner (e.g. postgres)."
      };
    }
    ...
    try {
      ...
      console.error("[ticket-trash] Automatic migration failed:", lastFailureMessage, "\n→ Restart the server or run: npm run schema:incremental");
      return {
        ready: false,
        error: lastFailureMessage
      };
