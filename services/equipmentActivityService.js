...
export function parseActivityDateInput(raw, { endOfDay = false } = {}) {
...
export function resolveActivityDateRange(startDate, endDate) {
  const end = parseActivityDateInput(endDate, { endOfDay: true }) || new Date();
  const start =
    parseActivityDateInput(startDate, { endOfDay: false }) ||
...
function buildStatsFromTickets(tickets) {
  const byStatus = { new: 0, in_progress: 0, pending: 0, resolved: 0, closed: 0 };
  ...
  for (const ticket of tickets) {
    ...
    if (byPriority[priorityKey] !== undefined) byPriority[priorityKey] += 1;
...
function buildStatsFromEvents(events) {
  ...
  for (const event of events) {
    ...
    if (!Number.isNaN(endMs) && endMs >= now) upcoming += 1;
    else past += 1;
...
async function fetchPlanningEvents({ equipmentId, clientId, start, end }) {
...
async function fetchLinkedTickets({ equipmentId, clientId, start, end }) {
  const hasEquipmentInfo = await hasEquipmentInfoColumn();
  const { hasDeletedAt, hasIsDeleted } = await hasTicketDeletedColumns();
  ...
  where.push(
    `(COALESCE(t.updated_at, t.created_at) >= $${index} AND COALESCE(t.updated_at, t.created_at) <= $${index + 1})`
  );
...
export async function loadEquipmentActivity({ equipmentId, clientId, startDate, endDate }) {
  const parsedEquipmentId = parseEquipmentActivityId(equipmentId);
  if (!parsedEquipmentId) {
    const error = new Error("ID périphérique invalide");
  ...
  if (!parsedClientId || Number.isNaN(parsedClientId)) {
    const error = new Error("clientId requis");
    error.statusCode = 400;
    throw error;
  }

  const { start, end } = resolveActivityDateRange(startDate, endDate);

  const [events, tickets] = await Promise.all([
    fetchPlanningEvents({ equipmentId: parsedEquipmentId, clientId: parsedClientId, start, end }),
    fetchLinkedTickets({ equipmentId: parsedEquipmentId, clientId: parsedClientId, start, end }),
  ]);
