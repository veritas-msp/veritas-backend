...
import {
  buildRunbookComment,
  resolveRunbookForCriterion,
  resolveRunbookTicketPriority,
  resolveRunbookTags,
} from "./monitoringRunbooks.js";
...
import {
  criteriaToActiveMap,
  diffCriteriaTransitions,
  evaluateEquipmentSupervisionCriteria,
} from "../utils/equipmentSupervisionEvaluator.js";
...
function buildCriterionDescription({
  ...
  const lines = [
    ...
    `Source : ${source === "rmm" ? "Agent RMM" : source === "external" ? "Événement externe" : "Supervision"}`,
  ];

  if (detail && typeof detail === "object") {
    if (detail.pendingCount) lines.push(`Mises à jour en attente : ${detail.pendingCount}`);
    if (detail.pct != null) lines.push(`Utilisation disque : ${detail.pct}%`);
    if (detail.drive) lines.push(`Volume : ${detail.drive}`);
    if (detail.critServices > 0) lines.push(`Services critiques : ${detail.critServices}`);
    if (detail.warnServices > 0) lines.push(`Services warning : ${detail.warnServices}`);
  }

  lines.push("", "Ce ticket a été créé automatiquement suite à un critère de surveillance.");
  return lines.join("\n");
}
...
async function hasTicketColumn(columnName) {
  const result = await pool.query(
    `SELECT 1 FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name = 'v_b_tickets' AND column_name = $1 LIMIT 1`,
    [columnName]
  );
...
async function createCriterionAlertTicket({
  ...
  const title = `[Surveillance] ${equipmentName || "Périphérique"} — ${label}`;
  ...
  const result = await pool.query(
    `INSERT INTO v_b_tickets (${cols.join(", ")}, created_at, updated_at)
     VALUES (${placeholders.join(", ")}, NOW(), NOW())
     RETURNING id, ticket_number, title`,
    hasEquipmentInfo && hasMonitoringMeta
      ? vals
      : hasEquipmentInfo
        ? vals
        : hasMonitoringMeta
  ...
  if (tags.length) {
    await pool.query(
      `UPDATE v_b_tickets SET tags = $1::text[], updated_at = NOW() WHERE id = $2::uuid`,
      [tags, ticket.id]
    ).catch(() => {});
  }
  ...
  await pool.query(
    ...
    [ticket.id, buildLinkedEquipmentComment({ equipmentId, equipmentName, equipmentFamily, clientId })]
  );

  let runbookForComment = runbook ? { ...runbook } : null;
  let aiNotes = null;
  try {
    ...
    if (enriched?.checklist?.length) {
      runbookForComment = {
        ...(runbookForComment || { title: label, priority: "normal", tags: [] }),
  ...
  await dispatchNotificationEvent({
    ...
    context: { ticket, entreprise: { id: String(clientId) } },
  }).catch(() => {});
...
/**
 * Evaluates a monitoring status change and creates a support ticket when needed.
 */
export async function evaluateEquipmentCriteriaAlerts({
  ...
  const previousMap =
    previousCriteriaMap ||
    (settings?.lastKnownCriteria && typeof settings.lastKnownCriteria === "object"
      ? settings.lastKnownCriteria
      : {});

  const activeMap = criteriaToActiveMap(activeCriteria);
  const { activated, resolved } = diffCriteriaTransitions(previousMap, activeMap);
  ...
  if (isClientMonitoringAlertsSuspended(clientPolicy)) {
    ...
    return { skipped: true, reason: "client_suspended", activated: [], resolved };
  }
  ...
  if (!areMonitoringAlertsEnabled(settings)) {
    ...
    return { skipped: true, reason: "disabled", activated: [], resolved };
  }
  ...
  for (const criterionKey of activated) {
    ...
    if (!allowed) {
      skipped.push({ criterionKey, reason: "global_rule_disabled" });
      continue;
    }
  ...
  const aggregateStatus = activeMap.monitor_critical || activeMap.agent_offline || activeMap.disk_critical
    ? "critical"
    : Object.keys(activeMap).length
      ? "warning"
      : "ok";
...
async function handleCriterionRecovery({
  ...
  const label = getCriterionLabel(criterionKey) || criterionKey;
  const comment = [
    "✅ Récupération automatique détectée",
    "",
    `Le critère « ${label} » n'est plus actif sur ce périphérique.`,
    autoRes.requireAgentValidation
      ? "Veuillez valider la résolution et clôturer le ticket si l'intervention est terminée."
      : "Le ticket peut être clôturé si aucune autre action n'est requise.",
  ].join("\n");

  const openTickets = await pool.query(
    `SELECT id, ticket_number, monitoring_meta, equipment_info
     FROM v_b_tickets
     WHERE client_id = $1
  ...
  for (const ticket of openTickets.rows) {
    ...
    if (autoRes.requireAgentValidation) {
      ...
      await pool.query(
        `UPDATE v_b_tickets SET status = 'resolved', resolved_at = NOW(), updated_at = NOW() WHERE id = $1`,
        [ticket.id]
      );
    }
    ...
    await recordMonitoringEvent({
      ...
      criterionKey,
      ticketId: ticket.id,
      payload: { autoResolution: true },
    });
  }
}

/** Compatibility: evaluation by aggregated status (delegates to criteria). */
export async function evaluateMonitoringAlert({
  ...
  if (status === "critical") activeCriteria.push({ key: "monitor_critical", detail: details });
  else if (status === "warning") activeCriteria.push({ key: "monitor_warning", detail: details });
  else if (status === "offline") activeCriteria.push({ key: "agent_offline", detail: details });
  else if (status === "unmapped") activeCriteria.push({ key: "unmapped", detail: details });
  else if (status === "no_data") activeCriteria.push({ key: "no_data", detail: details });
...
export async function evaluateInventoryItem(item, { offlineAlertThresholdMinutes, thresholds, rules } = {}) {
