...
export async function findOrCreateIncidentGroup({
  ...
  const existing = await pool.query(
    `SELECT id, parent_ticket_id, equipment_count
     ...
     LIMIT 1`,
    [clientId, criterionKey, since]
  );

  if (existing.rows[0]?.id) {
    const groupId = existing.rows[0].id;
    await pool.query(
      `UPDATE v_b_monitoring_incident_groups
  ...
  const title = `[Corrélation] ${getCriterionLabel(criterionKey)} — client #${clientId}`;
  const inserted = await pool.query(
    `INSERT INTO v_b_monitoring_incident_groups
     ...
     RETURNING id`,
    [clientId, title.slice(0, 255), criterionKey, parentTicketId]
  );
  ...
  await pool.query(
    `UPDATE v_b_tickets
     SET monitoring_meta = COALESCE(monitoring_meta, '{}'::jsonb) || $1::jsonb,
         updated_at = NOW()
     WHERE id = $2::uuid`,
    [JSON.stringify({ incidentGroupId: String(incidentGroupId) }), ticketId]
  );
}
