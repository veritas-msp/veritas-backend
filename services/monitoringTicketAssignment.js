...
export async function applyMonitoringTicketAssignment({
  ticketId,
  clientId,
  criterionKey,
  config
  ...
  await pool.query(`UPDATE v_b_tickets SET assigned_user_id = $1::uuid, updated_at = NOW() WHERE id = $2::uuid`, [primaryId, ticketId]);
  for (const userId of userIds) {
    await pool.query(`INSERT INTO v_b_ticket_assignees (ticket_id, user_id, created_at)
       VALUES ($1::uuid, $2::uuid, NOW())
       ON CONFLICT (ticket_id, user_id) DO NOTHING`, [ticketId, userId]).catch(() => {});
  }
  return {
    assigned: true,
