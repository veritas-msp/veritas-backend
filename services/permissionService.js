...
import { ALL_PERMISSION_KEYS, PERMISSION_CATALOG, MODULE_FLAG_TO_GROUPS, defaultPermissionsForProfile, permissionKey } from "../config/permissionCatalog.js";
...
export async function getProfilePermissions(profileName) {
  ...
  try {
    ...
    if (rows.length === 0) {
      if (preset) {
        perms = preset;
        persistProfileMatrix(name, preset).catch(err => console.warn("[permissions] Persist preset failed:", err.message));
      } else {
      ...
      if (preset && !hasAnyViewPermission(perms)) {
        perms = preset;
        persistProfileMatrix(name, preset).catch(err => console.warn("[permissions] Heal preset failed:", err.message));
      }
    }
  } catch (err) {
