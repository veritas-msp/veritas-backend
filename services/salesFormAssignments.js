...
export async function loadAssignmentsByFormIds(formIds = []) {
  ...
  const [profiles, users, teams] = await Promise.all([
    ...
    pool.query(
      `SELECT fu.form_id, fu.user_id, u.email, u.username
    ...
    pool.query(
      `SELECT ft.form_id, ft.team_id, t.name AS team_name
...
export async function syncFormAssignments(formId, assignments = {}) {
  ...
  for (const profileName of profileNames) {
    await pool.query(
      `INSERT INTO v_b_sales_form_profiles (form_id, profile_name) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
      [formId, profileName]
    );
  }
  for (const userId of userIds) {
    await pool.query(
      `INSERT INTO v_b_sales_form_users (form_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
      [formId, userId]
    );
  }
  for (const teamId of teamIds) {
    await pool.query(
      `INSERT INTO v_b_sales_form_teams (form_id, team_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
      [formId, teamId]
    );
  }
