...
function normalizeRule(raw = {}, index = 0) {
  const conditions = Array.isArray(raw.conditions)
    ? raw.conditions.map(normalizeCondition).filter((c) => c.fieldKey)
    : [];
  return {
...
export function describeTicketTargets(config = {}) {
  ...
  if (enabledRules.length === 0) return "Aucune cible";
  if (enabledRules.length === 1) {
    const rule = enabledRules[0];
    const parts = [rule.label];
    const targets = rule.targets;
    if (targets.priority) parts.push(`Priorité ${targets.priority}`);
    if (targets.assigneeUserIds.length) parts.push(`${targets.assigneeUserIds.length} assigné(s)`);
    return parts.join(" · ");
  }
  return `${enabledRules.length} cible(s) conditionnelle(s)`;
}
...
export async function loadFormTicketTargetsConfig(formId) {
  ...
  try {
    const result = await pool.query(
      `SELECT ticket_targets FROM v_b_sales_form_definitions WHERE id = $1 AND enabled = TRUE`,
      [String(formId)]
    );
...
/** @deprecated use loadFormTicketTargetsConfig */
...
async function hasTicketAssigneesTable() {
  const result = await pool.query(
    `SELECT to_regclass('public.v_b_ticket_assignees') IS NOT NULL AS ok`
  );
...
async function hasTicketWatchersTable() {
  const result = await pool.query(
    `SELECT to_regclass('public.v_b_ticket_watchers') IS NOT NULL AS ok`
  );
  return Boolean(result.rows[0]?.ok);
}
...
export async function applyFormTicketTargets(ticketId, targets = {}) {
  ...
  return { assigneeUserIds, watcherUserIds };
}
...
export function buildTicketTitle(baseTitle, rule) {
  ...
  if (!suffix || suffix === "Ticket principal") return baseTitle;
