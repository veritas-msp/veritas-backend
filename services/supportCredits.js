...
export function isSalesTicket(type, category) {
  ...
  return (
    normalizedType === "demande" &&
    (normalizedCategory.startsWith("prestation-") || normalizedCategory.startsWith("installation-"))
  );
}

export function shouldConsumeSupportCredit({ type, category, clientId }) {
  if (!clientId) return false;
  return !isSalesTicket(type, category);
}

export async function resolveClientIdForTicket({ clientId, requesterContactId }) {
...
async function syncClientBalance(dbClient, clientId) {
  ...
  if (hasPacks) {
    const packsResult = await dbClient.query(
    ...
    const legacy = await dbClient.query(
      "SELECT balance FROM v_b_client_support_credits WHERE client_id = $1",
      [clientId]
    );
  ...
  await dbClient.query(
    `INSERT INTO v_b_client_support_credits (client_id, balance, updated_at)
     VALUES ($1, $2, NOW())
...
function createInsufficientCreditsError(balance) {
  const err = new Error("Crédits support insuffisants");
...
async function pickPackForDebit(dbClient, clientId) {
  ...
  const result = await dbClient.query(
    `SELECT id, remaining_amount, valid_from, valid_until
     FROM v_b_client_support_credit_packs
     WHERE client_id = $1
       AND archived_at IS NULL
       AND remaining_amount > 0
...
async function applyCreditDelta(dbClient, clientId, delta, { ticketId, note, userId, kind, packId = null }) {
  ...
  const ledgerResult = await dbClient.query(
    `INSERT INTO v_b_client_support_credit_ledger
       (client_id, delta, balance_after, kind, ticket_id, pack_id, note, created_by, created_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW())
     RETURNING *`,
    [clientId, delta, newBalance, kind, ticketId || null, packId, note || null, userId || null]
  );

  await syncClientBalance(dbClient, clientId);
  return { balance: newBalance, entry: ledgerResult.rows[0] };
}
...
export async function listCreditPacks(clientId) {
  ...
  const result = await pool.query(
    `SELECT p.*, u.username, u.email
     ...
     ORDER BY p.valid_until ASC NULLS LAST, p.created_at DESC`,
    [clientId]
  );
...
export async function getSupportCreditSummary(clientId) {
  const [balance, packs, ledgerResult] = await Promise.all([
    getSupportCreditBalance(clientId),
    listCreditPacks(clientId),
    pool.query(
      `SELECT l.id, l.client_id, l.delta, l.balance_after, l.kind, l.ticket_id, l.pack_id, l.note,
...
export async function createCreditPack(
  clientId,
  { amount, validFrom = null, validUntil = null, label = null, note, userId } = {}
) {
  const parsedAmount = Number(amount);
  if (!Number.isInteger(parsedAmount) || parsedAmount <= 0) {
    const err = new Error("Le nombre de tickets doit être un entier positif");
    err.status = 400;
    throw err;
  }
  ...
  const hasPacks = await hasPacksTable();
  if (!hasPacks) {
    const err = new Error("Les carnets de tickets ne sont pas encore installés (migration requise)");
    err.status = 503;
    throw err;
  }
  ...
  try {
    ...
    const packResult = await dbClient.query(
      ...
      [
        clientId,
        label || `Carnet ${parsedAmount} ticket(s)`,
        parsedAmount,
        validFrom || null,
        validUntil || null,
        note || null,
    ...
    return {
      pack: { ...pack, status: packStatus(pack) },
      balance,
...
export async function listAllSupportCreditPacks() {
  ...
  const result = await pool.query(
    `SELECT p.id, p.client_id, p.label, p.initial_amount, p.remaining_amount,
...
export async function updateCreditPack(clientId, packId, updates = {}, userId = null) {
  ...
  try {
    ...
    if (!existing) {
      const err = new Error("Carnet introuvable");
      err.status = 404;
      throw err;
    }
    ...
    const nextLabel =
      updates.label !== undefined ? String(updates.label || "").trim() || existing.label : existing.label;
    const nextNote =
      updates.note !== undefined ? String(updates.note || "").trim() || null : existing.note;
    const nextValidFrom =
    ...
    let nextInitial =
      updates.initial_amount !== undefined ? Number(updates.initial_amount) : Number(existing.initial_amount);
    let nextRemaining =
      updates.remaining_amount !== undefined
        ? Number(updates.remaining_amount)
        : Number(existing.remaining_amount);

    if (!Number.isInteger(nextInitial) || nextInitial <= 0) {
      const err = new Error("Le nombre initial de tickets doit être un entier positif");
      err.status = 400;
      throw err;
    }
    if (!Number.isInteger(nextRemaining) || nextRemaining < 0) {
      const err = new Error("Le solde restant doit être un entier positif ou nul");
      err.status = 400;
      throw err;
    }
    if (nextRemaining > nextInitial) {
      const err = new Error("Le solde restant ne peut pas dépasser le nombre initial");
      err.status = 400;
      throw err;
    }
    if (nextInitial < consumed) {
      const err = new Error(
        `Le nombre initial ne peut pas être inférieur aux tickets déjà consommés (${consumed})`
      );
      err.status = 400;
      throw err;
    }
    ...
    const updateResult = await dbClient.query(
      `UPDATE v_b_client_support_credit_packs
       SET label = $1,
           initial_amount = $2,
           remaining_amount = $3,
           valid_from = $4,
       ...
       WHERE id = $7
       RETURNING *`,
      [nextLabel, nextInitial, nextRemaining, nextValidFrom, nextValidUntil, nextNote, packId]
    );
    ...
    return { pack: { ...pack, status: packStatus(pack) }, balance };
  } catch (err) {
    await dbClient.query("ROLLBACK");
    throw err;
  } finally {
  ...
  try {
    ...
    if (Number(existing.remaining_amount) > 0) {
      await dbClient.query(
        `INSERT INTO v_b_client_support_credit_ledger
           (client_id, delta, balance_after, kind, ticket_id, pack_id, note, created_by, created_at)
         VALUES ($1, $2, $3, 'adjustment', NULL, $4, $5, $6, NOW())`,
        [
          clientId,
          -Number(existing.remaining_amount),
          balance,
          packId,
          `Archivage carnet : ${existing.label || "Carnet"}`,
          userId,
        ]
      );
    }
...
export function normalizeSupportCreditDebits(input, { consumeSupportCredit = false } = {}) {
...
async function listTicketDebitEntries(ticketId, dbClient = pool) {
  const result = await dbClient.query(
    `SELECT l.*, p.label AS pack_label, t.ticket_number
     FROM v_b_client_support_credit_ledger l
     LEFT JOIN v_b_client_support_credit_packs p ON p.id = l.pack_id
     LEFT JOIN v_b_tickets t ON t.id = l.ticket_id
...
export async function getTicketCreditStatus(ticket) {
  ...
  const [balance, packs, consumed, refunded, debitEntries] = await Promise.all([
    getSupportCreditBalance(clientId),
    listCreditPacks(clientId),
    isTicketCreditDebited(ticket.id),
    isTicketCreditRefunded(ticket.id),
    listTicketDebitEntries(ticket.id),
  ]);

  const totalDebited = debitEntries.reduce(
...
export async function consumeCreditsForTicket(ticketId, userId, debits = []) {
  ...
  if (normalizedDebits.length === 0) {
    return { skipped: true, reason: "no_debits" };
  }

  const ticketResult = await pool.query(
    "SELECT id, client_id, type, category, ticket_number FROM v_b_tickets WHERE id = $1",
    [ticketId]
  );
  ...
  const clientId = ticket.client_id ? Number(ticket.client_id) : null;
  if (!shouldConsumeSupportCredit({ type: ticket.type, category: ticket.category, clientId })) {
  ...
  const hasPacks = await hasPacksTable();
  const dbClient = await pool.connect();
  const noteBase = ticket.ticket_number ? `Résolution ticket #${ticket.ticket_number}` : "Résolution ticket support";
  ...
  try {
    ...
    for (const debit of normalizedDebits) {
      ...
      if (hasPacks && debit.packId) {
        ...
        applied.push({ packId: debit.packId, amount, balance: result.balance, entry: result.entry });
        continue;
      }
      ...
      if (hasPacks && !debit.packId) {
        ...
        while (remainingToDebit > 0) {
          ...
          applied.push({ packId: pack.id, amount: packDebit, balance: result.balance, entry: result.entry });
      ...
      applied.push({ packId: null, amount, balance: result.balance, entry: result.entry });
    }
      ...
      if (debitEntry.pack_id) {
        await dbClient.query(
          `UPDATE v_b_client_support_credit_packs
           ...
           WHERE id = $1`,
          [debitEntry.pack_id, amount]
        );
      }

      const result = await applyCreditDelta(dbClient, debitEntry.client_id, amount, {
        ticketId,
        note: debitEntry.ticket_number
          ? `Remboursement ticket #${debitEntry.ticket_number}`
          : debitEntry.pack_label
            ? `Remboursement carnet : ${debitEntry.pack_label}`
            : "Remboursement crédit ticket",
        userId,
        kind: "refund",
...
export async function handleTicketStatusCreditChange({
  ...
  const debits = normalizeSupportCreditDebits(supportCreditDebits, { consumeSupportCredit });

  if (
    debits.length > 0 &&
    (normalizedNew === "resolved" || normalizedNew === "closed") &&
    normalizedOld !== "resolved" &&
    normalizedOld !== "closed"
  ) {
  ...
  if (
    refundSupportCredit &&
    (normalizedOld === "resolved" || normalizedOld === "closed") &&
    normalizedNew !== "resolved" &&
    normalizedNew !== "closed"
  ) {
