...
async function hasLegacyAutomationColumn(columnName) {
  ...
  try {
    const result = await pool.query(
      ...
      [LEGACY_AUTOMATION_TABLE, columnName]
    );
...
async function writeLegacyAutomationColumn(tableName, value) {
  ...
  const update = await pool.query(
    `UPDATE ${LEGACY_AUTOMATION_TABLE}
     SET ${columnName} = $1${cast},
         updated_at = NOW()
     WHERE id = $2`,
    [payload, SINGLETON_ID]
  );
  if (update.rowCount === 0) {
    await pool.query(
      `INSERT INTO ${LEGACY_AUTOMATION_TABLE} (id, ${columnName}, created_at, updated_at)
       VALUES ($1, $2${cast}, NOW(), NOW())`,
...
async function readNotificationLogsRows(defaultValue = []) {
  ...
  const result = await pool.query(
    `SELECT id, source, status, channel, element, message, created_at${
      hasEnterpriseId ? ", enterprise_id" : ""
    }
...
async function writeNotificationLogsRows(logs = []) {
  ...
  for (const log of safeLogs) {
    if (hasEnterpriseId) {
      await pool.query(
        `INSERT INTO ${TABLES.notificationLogs}
          (id, source, status, channel, element, message, created_at, enterprise_id)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [
          String(log?.id || `notif-log-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`),
          String(log?.source || ""),
          String(log?.status || ""),
          String(log?.channel || ""),
          String(log?.element || ""),
          String(log?.message || ""),
          normalizeTimestampForSql(log?.createdAt),
          String(log?.enterpriseId || ""),
        ]
      );
    } else {
      await pool.query(
...
export async function loadTicketAutomationRawConfig() {
  const [
    ...
    readJsonTable(TABLES.commentTemplates, []),
    readJsonTable(TABLES.macros, []),
    readJsonTable(TABLES.emailInboxes, []),
    readJsonTable(TABLES.exclusionRules, []),
    readJsonTable(TABLES.autoReplyRules, []),
    readJsonTable(TABLES.autoReplyTemplate, ""),
    readJsonTable(TABLES.scheduledAlertRules, []),
    readJsonTable(TABLES.mailCollectors, []),
    readJsonTable(TABLES.mailCollectSettings, {}),
    readJsonTable(TABLES.notificationEvents, {}),
    readJsonTable(TABLES.notificationWebhooks, []),
    readJsonTable(TABLES.notificationTemplates, []),
    readNotificationLogsRows([]),
  ]);

  const eventConfig =
    notificationEvents && typeof notificationEvents === "object" && !Array.isArray(notificationEvents)
      ? notificationEvents
      : {};
...
export async function saveTicketAutomationRawConfig(config = {}) {
  const settings =
    config?.notificationSettings && typeof config.notificationSettings === "object"
  ...
  await Promise.all([
    writeJsonTable(TABLES.commentTemplates, Array.isArray(config?.commentTemplates) ? config.commentTemplates : []),
    writeJsonTable(TABLES.macros, Array.isArray(config?.macros) ? config.macros : []),
    writeJsonTable(TABLES.emailInboxes, Array.isArray(config?.emailInboxes) ? config.emailInboxes : []),
    writeJsonTable(TABLES.exclusionRules, Array.isArray(config?.exclusionRules) ? config.exclusionRules : []),
    writeJsonTable(TABLES.autoReplyRules, Array.isArray(config?.autoReplyRules) ? config.autoReplyRules : []),
    writeJsonTable(TABLES.autoReplyTemplate, String(config?.autoReplyTemplate || "")),
    writeJsonTable(
      TABLES.scheduledAlertRules,
      Array.isArray(config?.scheduledAlertRules) ? config.scheduledAlertRules : []
    ),
    writeJsonTable(TABLES.mailCollectors, Array.isArray(config?.mailCollectors) ? config.mailCollectors : []),
    writeJsonTable(
      TABLES.mailCollectSettings,
      config?.mailCollectSettings && typeof config.mailCollectSettings === "object" && !Array.isArray(config.mailCollectSettings)
        ? config.mailCollectSettings
        : {}
    ),
  ]);
  ...
  const optionalSaveResults = await Promise.allSettled([
    ...
    writeJsonTable(TABLES.notificationTemplates, Array.isArray(templates) ? templates : []),
...
export async function loadNotificationSettingsRaw() {
  const [eventConfig, webhooks, templates, logs] = await Promise.all([
    readJsonTable(TABLES.notificationEvents, {}),
    readJsonTable(TABLES.notificationWebhooks, []),
    readJsonTable(TABLES.notificationTemplates, []),
    readNotificationLogsRows([]),
