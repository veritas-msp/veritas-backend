...
export const RESOLUTION_AUTO_CLOSE_PREFIX = "[Resolution auto-clôture]";
export const RESOLUTION_CLIENT_REJECTION_PREFIX = "[Resolution client]";
export const RESOLUTION_CLIENT_ACCEPT_PREFIX = "[Resolution client acceptée]";
...
export async function hasResolutionValidationTable() {
  ...
  const result = await pool.query(
    `SELECT to_regclass('v_b_ticket_resolution_validations') IS NOT NULL AS has_table`
  );
...
export async function getTicketResolutionValidation(ticketId) {
  ...
  const result = await pool.query(
    `SELECT id, ticket_id, resolution_reason, intervention_type, action_type,
...
export async function ensureTicketStatusMatchesValidation(ticketId) {
  ...
  const ticketResult = await pool.query(
    `SELECT id, status FROM v_b_tickets WHERE id = $1 LIMIT 1`,
    [ticketId]
  );
  ...
  const note =
    validation.outcome === "auto_closed"
      ? "Synchronisation — clôture automatique validation client"
      : "Synchronisation — validation client acceptée";
...
export async function markResolutionValidationReopened(ticketId) {
  ...
  const result = await pool.query(
    `DELETE FROM v_b_ticket_resolution_validations
     WHERE ticket_id = $1
       AND outcome IN ('pending', 'accepted', 'auto_closed')`,
    [ticketId]
  );
...
async function insertStatusHistory(ticketId, oldStatus, newStatus, userId, note) {
  await pool.query(
    ...
    [ticketId, oldStatus || null, newStatus, userId || null, note]
  );
}
...
async function addPublicComment(ticketId, userId, content) {
  const result = await pool.query(
    `INSERT INTO v_b_ticket_comments (ticket_id, author_user_id, content, is_internal, created_at)
     ...
     RETURNING id, ticket_id, author_user_id, content, is_internal, created_at`,
    [ticketId, userId || null, content]
  );
...
async function changeTicketStatus(ticketId, newStatus, userId, note) {
  const existing = await pool.query(
    `SELECT id, status, client_id, ticket_number, title
     FROM v_b_tickets WHERE id = $1 LIMIT 1`,
    [ticketId]
  );
...
export async function resolveTicketWithClientValidation({
  ...
  const commentContent =
    `${RESOLUTION_COMMENT_PREFIX} [${trimmedIntervention}] [${trimmedAction}] ${trimmedReason}`;
  ...
  if (status !== "resolved") {
    ...
    await insertStatusHistory(
      ...
      "resolved",
      userId,
      "Résolution avec validation client"
    );
    await dispatchNotificationEvent({
      source: "tickets",
      element: "resolved",
      enterpriseId: String(ticket.client_id || ""),
      context: { ticket: { id: ticketId }, entreprise: { id: String(ticket.client_id || "") } },
    }).catch(() => {});
    await notifyInAppTicketStatusChanged({
      ticketId,
      newStatus: "resolved",
  ...
  const validationResult = await pool.query(
    `INSERT INTO v_b_ticket_resolution_validations
...
export async function submitPortalResolutionValidation({
  ...
  if (accepted) {
    const acceptComment = trimmedMessage
      ? `${RESOLUTION_CLIENT_ACCEPT_PREFIX} ${trimmedMessage}`
      : `${RESOLUTION_CLIENT_ACCEPT_PREFIX} Le client a confirmé que la solution proposée répond à sa demande.`;
    ...
    try {
      ...
      await db.query(
        `INSERT INTO v_b_ticket_comments (ticket_id, author_user_id, content, is_internal, created_at)
         VALUES ($1, $2, $3, false, NOW())`,
        [ticketId, userId || null, acceptComment]
      );

      const lockedTicket = await db.query(
        `SELECT id, status, client_id, ticket_number, title
         FROM v_b_tickets
         WHERE id = $1
         FOR UPDATE`,
      ...
      const statusUpdate = await db.query(
        `UPDATE v_b_tickets
         SET status = 'closed',
             ...
             updated_at = NOW()
         WHERE id = $1
         RETURNING id, status, client_id, ticket_number, title`,
        [ticketId]
      );
      ...
      if (oldStatus !== "closed") {
        await db.query(
          ...
          [
            ...
            "closed",
            userId || null,
            "Validation client — solution acceptée",
          ]
        );
      }
    ...
    await dispatchNotificationEvent({
      ...
      context: {
        ticket: closedTicket,
        entreprise: { id: String(closedTicket?.client_id || ticket.client_id || "") },
      },
    }).catch(() => {});
    ...
    return {
      outcome: "accepted",
      status: "closed",
      validation: { ...validation, outcome: "accepted", isPending: false, respondedAt: now },
    };
  }

  const rejectionText =
    trimmedMessage ||
    "Le client indique que le problème n'est pas résolu et souhaite une nouvelle prise en charge.";

  await addPublicComment(
    ticketId,
    userId,
    `${RESOLUTION_CLIENT_REJECTION_PREFIX} ${rejectionText}`
  );
  ...
  await pool.query(
    `UPDATE v_b_ticket_resolution_validations
     SET outcome = 'rejected',
         ...
         updated_at = NOW()
     WHERE ticket_id = $1`,
    [ticketId, userId || null, rejectionText]
  );

  await changeTicketStatus(ticketId, "in_progress", userId, "Validation client — solution refusée");
...
export async function autoCloseExpiredResolutionValidations() {
  ...
  const pendingResult = await pool.query(
    `SELECT v.ticket_id, v.auto_close_at, t.status, t.client_id
  ...
  for (const row of pendingResult.rows) {
    try {
      await addPublicComment(
        row.ticket_id,
        null,
        `${RESOLUTION_AUTO_CLOSE_PREFIX} Le ticket a été clos automatiquement faute de réponse du client sous ${VALIDATION_AUTO_CLOSE_HOURS} heures.`
      );

      await pool.query(
        `UPDATE v_b_ticket_resolution_validations
         SET outcome = 'auto_closed', responded_at = NOW(), updated_at = NOW()
         WHERE ticket_id = $1 AND outcome = 'pending'`,
        [row.ticket_id]
      );
      ...
      await changeTicketStatus(
        ...
        "closed",
        null,
        "Clôture automatique — validation client expirée (48 h)"
      );
      closed += 1;
    } catch (err) {
