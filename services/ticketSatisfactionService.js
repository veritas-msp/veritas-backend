...
import {
  TICKET_SATISFACTION_CRITERIA,
  computeSatisfactionAverage,
  normalizeSatisfactionRatingsInput,
  resolveStoredRatings,
} from "./ticketSatisfactionCriteria.js";
...
export async function hasSatisfactionTable() {
  ...
  const result = await pool.query(
    `SELECT to_regclass('v_b_ticket_satisfaction') IS NOT NULL AS has_table`
  );
...
export async function enrichTicketRowsWithSatisfaction(rows = []) {
  ...
  return rows.map((row) => {
    ...
    return satisfaction ? { ...row, satisfaction } : row;
  });
}
...
export async function submitPortalTicketSatisfaction({
  ...
  const normalizedRatings =
    normalizeSatisfactionRatingsInput(ratings) ||
    (Number.isInteger(Number(rating)) && Number(rating) >= 1 && Number(rating) <= 5
      ? normalizeSatisfactionRatingsInput(
          Object.fromEntries(
            TICKET_SATISFACTION_CRITERIA.map(({ key }) => [key, Number(rating)])
          )
        )
      : null);
  ...
  const ticketResult = await pool.query(
    `SELECT id, client_id, status, ticket_number, title, assigned_user_id
     FROM v_b_tickets
  ...
  const existing = await pool.query(
    `SELECT id FROM v_b_ticket_satisfaction WHERE ticket_id = $1 LIMIT 1`,
    [ticketId]
  );
  ...
  const insertResult = ratingsColumnExists
    ? await pool.query(
        `INSERT INTO v_b_ticket_satisfaction (ticket_id, rating, ratings, message, author_user_id, created_at)
         VALUES ($1, $2, $3::jsonb, $4, $5, NOW())
         RETURNING id, ticket_id, rating, ratings, message, author_user_id, created_at`,
        [ticketId, overallRating, JSON.stringify(normalizedRatings), trimmedMessage || null, userId || null]
      )
    : await pool.query(
        `INSERT INTO v_b_ticket_satisfaction (ticket_id, rating, message, author_user_id, created_at)
         VALUES ($1, $2, $3, $4, NOW())
         RETURNING id, ticket_id, rating, message, author_user_id, created_at`,
        [ticketId, overallRating, trimmedMessage || null, userId || null]
      );

  const created = insertResult.rows[0];
  const authorResult = userId
    ? await pool.query(
        `SELECT COALESCE(NULLIF(TRIM(username), ''), email, 'Client') AS display_name
         FROM v_b_users WHERE id = $1 LIMIT 1`,
        [userId]
      )
    : { rows: [] };
...
export async function updatePortalTicketSatisfaction({
  ...
  if (
    ...
    userId &&
    String(existing.author_user_id) !== String(userId)
  ) {
  ...
  const ratingsColumnExists = await hasRatingsColumn();
  const updateResult = ratingsColumnExists
    ? await pool.query(
...
function buildSatisfactionListWhere({
  ...
  if (scope === "mine" && userId) {
    ...
    if (hasAssignees) {
      where.push(
        `(t.assigned_user_id = ${userParam}::uuid OR EXISTS (
  ...
  if (search) {
    ...
    where.push(
      `(t.title ILIKE ${p} OR CAST(t.ticket_number AS TEXT) ILIKE ${p} OR s.message ILIKE ${p} OR c.name ILIKE ${p})`
    );
  }
...
async function applySoftDeleteClauses(where, params, alias = "t") {
  const { hasDeletedAt, hasIsDeleted } = await hasTicketSoftDeleteColumns();
...
function mapSatisfactionListRow(row, assignees = []) {
  const satisfaction = mapSatisfactionRow(row, { display_name: row.author_name });
  return {
    ...satisfaction,
    ticket: {
      id: row.ticket_id,
...
export async function countTicketSatisfactions({
  ...
  const { where, params } = buildSatisfactionListWhere({
    userId,
    scope,
    search,
    sentiment,
...
export async function listTicketSatisfactions({
  ...
  const [{ rows: countRows }, { rows }] = await Promise.all([
    pool.query(
      `SELECT COUNT(*)::int AS total ${baseFrom} WHERE ${where.join(" AND ")}`,
      countParams
    ),
    pool.query(
      `SELECT s.id,
  ...
  const items = rows.map((row) =>
    mapSatisfactionListRow(row, assigneesByTicket.get(String(row.ticket_id)) || [])
  );
