...
export async function listSolutionCatalog({ category = "", includeInactive = false } = {}) {
  ...
  const result = await pool.query(
    `SELECT id, category, label, display_order, is_active, created_at, updated_at
     FROM v_b_ticket_solution_catalog
     ${whereSql}
     ORDER BY category ASC, display_order ASC, label ASC`,
    values
  );
  return (result.rows || []).map(mapCatalogRow);
}

export async function createSolutionCatalogEntry({ category, label, displayOrder = 0, isActive = true } = {}) {
  ...
  const result = await pool.query(
    `INSERT INTO v_b_ticket_solution_catalog (category, label, display_order, is_active, created_at, updated_at)
     ...
     RETURNING id, category, label, display_order, is_active, created_at, updated_at`,
    [normalizedCategory, normalizedLabel, Number(displayOrder) || 0, isActive !== false]
  );
...
export async function updateSolutionCatalogEntry(id, patch = {}) {
  ...
  const result = await pool.query(
    `UPDATE v_b_ticket_solution_catalog
     SET ${updates.join(", ")}
     WHERE id = $${idx}
     RETURNING id, category, label, display_order, is_active, created_at, updated_at`,
    values
...
export async function deleteSolutionCatalogEntry(id) {
  await ensureTicketSolutionCatalogSchema();
  const result = await pool.query(
    `DELETE FROM v_b_ticket_solution_catalog WHERE id = $1 RETURNING id`,
    [id]
  );
