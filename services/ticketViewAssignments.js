...
function emptyAssignments() {
  return { profileNames: [], userIds: [], teamIds: [] };
}
...
export async function loadAssignmentsByViewIds(viewIds = []) {
  ...
  const [profiles, users, teams] = await Promise.all([
    ...
    pool.query(
      `SELECT vu.view_id, vu.user_id, u.email, u.username
    ...
    pool.query(
      `SELECT vt.view_id, vt.team_id, t.name AS team_name
...
export async function syncViewAssignments(viewId, assignments = {}) {
  ...
  for (const profileName of profileNames) {
    await pool.query(
      `INSERT INTO v_b_ticket_view_profiles (view_id, profile_name) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
      [viewId, profileName]
    );
  }
  for (const userId of userIds) {
    await pool.query(
      `INSERT INTO v_b_ticket_view_users (view_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
      [viewId, userId]
    );
  }
  for (const teamId of teamIds) {
    await pool.query(
      `INSERT INTO v_b_ticket_view_teams (view_id, team_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
      [viewId, teamId]
    );
  }
...
export async function getUserTeamIds(userId) {
  ...
  const result = await pool.query(
    `SELECT team_id FROM v_b_team_members WHERE user_id = $1`,
    [userId]
  );
...
export function hasAssignmentTargets(assignments = {}) {
  return (
    (assignments.profileNames || []).length > 0
    || (assignments.userIds || []).length > 0
    || (assignments.teamIds || []).length > 0
  );
}
