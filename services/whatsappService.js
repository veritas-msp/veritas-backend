...
function sanitizeDiskFilename(name, mimeType) {
  const base = String(name || "whatsapp-fichier")
    .replace(/[^\w.\-]+/g, "_")
    .slice(0, 120);
...
function inboundMediaFallbackLabel(mediaType) {
  ...
  if (mediaType === "video") return "(Vidéo WhatsApp)";
  return "(Fichier WhatsApp)";
}
...
export async function getWhatsAppConfig() {
  ...
  const hasCredentials =
    Boolean(settings.WHATSAPP_ACCESS_TOKEN) && Boolean(settings.WHATSAPP_PHONE_NUMBER_ID);
  const enabled =
    enabledFlag === "true" || (enabledFlag !== "false" && hasCredentials);
...
async function isMessageAlreadyProcessed(waMessageId) {
  ...
  const result = await pool.query(
    "SELECT 1 FROM v_b_whatsapp_processed_messages WHERE wa_message_id = $1 LIMIT 1",
    [waMessageId]
  );
...
async function markMessageProcessed(waMessageId) {
  ...
  await pool.query(
    `INSERT INTO v_b_whatsapp_processed_messages (wa_message_id, processed_at)
     VALUES ($1, NOW())
...
async function resolveContactByPhone(waPhone) {
  ...
  const result = await pool.query(
    `SELECT id, client_id, nom, prenom, telephone
     FROM v_b_contacts
...
async function findOpenTicketForPhone(waPhone) {
  const result = await pool.query(
    `SELECT w.ticket_id, t.ticket_number, t.status
...
async function linkConversation({ waPhone, ticketId, contactName }) {
  await pool.query(
    `INSERT INTO v_b_whatsapp_conversations (wa_phone, ticket_id, wa_contact_name, last_message_at, created_at)
     ...
     DO UPDATE SET
       ...
       last_message_at = NOW()`,
    [waPhone, ticketId, contactName || null]
  );
}

async function touchConversation(ticketId) {
  await pool.query(
    `UPDATE v_b_whatsapp_conversations SET last_message_at = NOW() WHERE ticket_id = $1`,
    [ticketId]
  );
}

function buildInboundComment({ contactName, waPhone, text }) {
  const label = [contactName, waPhone].filter(Boolean).join(" — ") || waPhone || "Client WhatsApp";
  return `[WhatsApp] ${label}\n\n${String(text || "").trim() || "(message vide)"}`;
}

async function insertTicketAttachment({ ticketId, commentId, fileName, relativePath, mimeType, fileSize }) {
  await pool.query(
    ...
    [ticketId, commentId || null, fileName, relativePath, mimeType || null, fileSize || 0]
  );
}

async function saveBufferAsTicketAttachment({ ticketId, commentId, buffer, mimeType, fileName }) {
  if (!buffer || buffer.length === 0) {
    throw new Error("Fichier WhatsApp vide.");
  }
  if (buffer.length > MAX_ATTACHMENT_BYTES) {
    throw new Error("Fichier WhatsApp trop volumineux (max 15 Mo).");
  }

  const safeName = sanitizeDiskFilename(fileName, mimeType);
  const ext = path.extname(safeName).toLowerCase();
  if (!ALLOWED_ATTACHMENT_EXTENSIONS.has(ext)) {
    throw new Error(`Type de fichier WhatsApp non supporté (${mimeType || ext || "inconnu"}).`);
  }
  ...
  return { fileName: safeName, relativePath, mimeType, fileSize: buffer.length };
}

async function createTicketFromWhatsApp({ waPhone, contactName, text, contact, withComment = false }) {
  ...
  const description = buildInboundComment({ contactName: displayName, waPhone, text });
  ...
  const result = await pool.query(
    `INSERT INTO v_b_tickets
     ...
     RETURNING id, ticket_number, client_id`,
    [title, description, contact?.client_id || null, contact?.id || null]
  );
  const ticket = result.rows[0];
  if (!ticket?.id) return null;

  await linkConversation({ waPhone, ticketId: ticket.id, contactName: displayName });
  ...
  await dispatchNotificationEvent({
    ...
    context: {
      ...
      entreprise: { id: String(ticket.client_id || "") },
    },
  }).catch(() => {});

  return { ...ticket, commentId };
}

async function addWhatsAppCommentToTicket({ ticketId, waPhone, contactName, text }) {
  const content = buildInboundComment({ contactName, waPhone, text });
  const result = await pool.query(
  ...
  await dispatchNotificationEvent({
    ...
    context: {
      ...
      entreprise: { id: String(ticketRow.rows[0]?.client_id || "") },
    },
  }).catch(() => {});
...
async function fetchWhatsAppMediaBuffer(mediaId, config) {
  ...
  if (!metaResponse.ok) {
    throw new Error(meta?.error?.message || "Impossible de récupérer les métadonnées média WhatsApp.");
  }

  const downloadUrl = meta.url;
  if (!downloadUrl) throw new Error("URL média WhatsApp absente.");

  const downloadResponse = await fetch(downloadUrl, {
    headers: { Authorization: `Bearer ${config.accessToken}` },
  });
  if (!downloadResponse.ok) {
    throw new Error("Échec du téléchargement du média WhatsApp.");
  }
...
async function downloadAndAttachWhatsAppMedia({ ticketId, commentId, media, config }) {
  const { buffer, mimeType } = await fetchWhatsAppMediaBuffer(media.mediaId, config);
  const fileName = media.filename || sanitizeDiskFilename("whatsapp-fichier", mimeType);
...
export async function processInboundWhatsAppMessage({
  ...
  if (!config.enabled) return { skipped: true, reason: "integration_disabled" };
  ...
  if (openTicket?.ticket_id) {
    ...
    if (!created?.id) {
      await markMessageProcessed(waMessageId);
      return { skipped: true, reason: "ticket_creation_failed" };
    }
    ticketId = created.id;
  ...
  await markMessageProcessed(waMessageId);
  return { action, ticketId, ticketNumber, hasMedia: Boolean(media) };
}
...
async function postWhatsAppMessagePayload(config, payload) {
  ...
  if (!response.ok) {
    const detail = data?.error?.message || response.statusText || "Erreur API WhatsApp";
...
export async function sendWhatsAppText({ to, text }) {
  const config = await getWhatsAppConfig();
  if (!config.enabled) {
    throw new Error("Intégration WhatsApp inactive ou non configurée.");
  }

  const recipient = normalizeWaPhone(to);
  const body = String(text || "").trim();
  if (!recipient) throw new Error("Numéro WhatsApp destinataire invalide.");
  if (!body) throw new Error("Le message WhatsApp ne peut pas être vide.");

  const truncated =
    body.length > MAX_WHATSAPP_TEXT_LENGTH
      ? `${body.slice(0, MAX_WHATSAPP_TEXT_LENGTH - 1)}…`
      : body;
...
async function uploadWhatsAppMedia({ buffer, mimeType, filename }) {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data?.error?.message || "Échec upload média WhatsApp.");
  }
  return data.id;
}

async function sendWhatsAppMediaMessage({ to, buffer, mimeType, filename, caption, mediaType }) {
  const config = await getWhatsAppConfig();
  const recipient = normalizeWaPhone(to);
  if (!recipient) throw new Error("Numéro WhatsApp destinataire invalide.");
  ...
  const safeCaption =
    trimmedCaption.length > MAX_WHATSAPP_CAPTION_LENGTH
      ? `${trimmedCaption.slice(0, MAX_WHATSAPP_CAPTION_LENGTH - 1)}…`
      : trimmedCaption;
...
export async function getWhatsAppPhoneForTicket(ticketId) {
  const result = await pool.query(
    "SELECT wa_phone FROM v_b_whatsapp_conversations WHERE ticket_id = $1 LIMIT 1",
    [ticketId]
  );
...
export async function maybeSendWhatsAppReplyForComment({
  ...
  if (isInternal) return { skipped: true, reason: "internal_comment" };

  const ticketResult = await pool.query(
    "SELECT id, channel FROM v_b_tickets WHERE id = $1",
    [ticketId]
  );
  const ticket = ticketResult.rows[0];
  if (!ticket || ticket.channel !== "whatsapp") {
    return { skipped: true, reason: "not_whatsapp_channel" };
  }

  const waPhone = await getWhatsAppPhoneForTicket(ticketId);
  if (!waPhone) return { skipped: true, reason: "missing_phone_mapping" };

  const plainText = stripHtmlToText(content);
  const ticketNumberResult = await pool.query(
    "SELECT ticket_number FROM v_b_tickets WHERE id = $1",
    [ticketId]
  );
  ...
  for (const attachment of fileRows) {
    ...
    if (buffer.length > MAX_ATTACHMENT_BYTES) {
      throw new Error(`Fichier trop volumineux pour WhatsApp : ${attachment.file_name || "pièce jointe"}`);
    }
    ...
    const ext = path.extname(String(fileName || "")).toLowerCase();
    if (!ALLOWED_ATTACHMENT_EXTENSIONS.has(ext)) {
      throw new Error(`Type de fichier non envoyable sur WhatsApp : ${fileName || ext}`);
    }
    ...
    if (messagePrefix && !textDelivered && !captionDelivered && mediaKind === "audio") {
      await sendWhatsAppText({ to: waPhone, text: messagePrefix });
      textDelivered = true;
    }

    const caption =
      canCaption && messagePrefix && !captionDelivered && !textDelivered
        ? messagePrefix.slice(0, MAX_WHATSAPP_CAPTION_LENGTH)
        : undefined;
  ...
  if (fileRows.length === 0) {
    if (!plainText) return { skipped: true, reason: "empty_content" };
    await sendWhatsAppText({ to: waPhone, text: messagePrefix });
    sent.push("text");
  ...
  if (sent.length === 0) {
    return { skipped: true, reason: "no_sendable_attachments" };
  }

  return { sent: true, items: sent };
}

export async function testWhatsAppConnection() {
  const config = await getWhatsAppConfig();
  if (!config.phoneNumberId || !config.accessToken) {
    throw new Error("Phone Number ID et token d'accès requis.");
  }
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data?.error?.message || "Échec de connexion à l'API WhatsApp.");
  }
