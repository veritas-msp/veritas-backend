...
export async function resolveBitdefenderCredentials({ clientId, bitdefenderTenantId } = {}) {
  if (bitdefenderTenantId) {
    ...
    if (result.rows.length === 0) {
      throw new Error('Tenant Bitdefender dédié introuvable');
    }
    const row = result.rows[0];
    const apiKey = decrypt(row.api_key_encrypted, row.iv, row.auth_tag);
    if (!apiKey) {
      throw new Error('Impossible de déchiffrer la clé API Bitdefender');
    }
    return {
      apiUrl: row.api_url,
      apiKey,
  ...
  if (!apiKey || !apiUrl) {
    throw new Error('API Bitdefender globale non configurée');
  }
