...
export async function getClientMonitoringAlertPolicy(clientId) {
  ...
  if (policy.suspensionType === "temporary" && policy.suspendedUntil && new Date(policy.suspendedUntil).getTime() <= Date.now()) {
...
export async function clearClientMonitoringAlertSuspension(clientId) {
  const id = Number(clientId);
  const result = await pool.query(`UPDATE v_b_clients
     ...
     RETURNING id, name,
               ...
               monitoring_alerts_suspended_at,
               monitoring_alerts_suspended_by,
               monitoring_alerts_suspension_reason`, [id]);
  const policy = mapClientPolicyRow(result.rows[0]);
  return policy ? {
    ...policy,
    suspended: false
...
export async function setClientMonitoringAlertSuspension({
  ...
  const id = Number(clientId);
  const normalized = suspensionType === "permanent" || suspensionType === "temporary" ? suspensionType : null;
  if (!normalized) {
    return clearClientMonitoringAlertSuspension(id);
  }
  const result = await pool.query(`UPDATE v_b_clients
     ...
     RETURNING id, name,
               ...
               monitoring_alerts_suspended_at,
               monitoring_alerts_suspended_by,
               monitoring_alerts_suspension_reason`, [id, normalized, normalized === "temporary" ? suspendedUntil : null, suspendedBy, suspensionReason || null]);
  const policy = mapClientPolicyRow(result.rows[0]);
  return policy ? {
    ...policy,
    suspended: isClientMonitoringAlertsSuspended(policy)
