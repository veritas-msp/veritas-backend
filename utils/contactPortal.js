...
export async function createPortalUserForContact(contact, password) {
  ...
  const { rows } = await pool.query(
    `INSERT INTO v_b_users (id, email, username, password_hash, role, client_id, contact_id, is_active, profile)
     VALUES ($1, $2, $3, $4, 'client', $5, $6, $7, NULL)
     RETURNING id, email, username, role, client_id, contact_id, is_active, last_login_at, created_at`,
    [randomUUID(), email, username, hash, contact.client_id, contact.id, contactActive]
  );
  return rows[0];
}
...
export async function resetPortalPassword(contactId, newPassword) {
  ...
  const { rows } = await pool.query(
    `UPDATE v_b_users SET password_hash = $1
     WHERE contact_id = $2 AND role = 'client'
     RETURNING id`,
    [hash, contactId]
  );
