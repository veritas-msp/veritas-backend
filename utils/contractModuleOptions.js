...
function slugifyModuleKey(value) {
  return String(value || "module").trim().replace(/\s+/g, "").replace(/[^a-zA-Z0-9_]/g, "").slice(0, 48) || "module";
}
function mapRow(row, {
  includeUsage = false
} = {}) {
...
export async function countClientsUsingModuleKey(moduleKey) {
  const result = await pool.query(`SELECT COUNT(*)::int AS count
     FROM v_b_clients
     WHERE options @> jsonb_build_object($1, true)`, [moduleKey]);
...
async function seedDefaultsIfEmpty() {
  ...
  for (const mod of DEFAULT_CONTRACT_MODULES) {
    await pool.query(`INSERT INTO v_b_contract_module_options (module_key, label, icon, enabled, sort_order)
       VALUES ($1, $2, $3, true, $4)`, [mod.module_key, mod.label, mod.icon, mod.sort_order]);
  }
}
export async function listContractModuleOptions({
  ...
  const result = await pool.query(`SELECT o.id, o.module_key, o.label, o.icon, o.enabled, o.sort_order, o.created_at, o.updated_at${usageSelect}
...
export async function createContractModuleOption(payload) {
  ...
  const exists = await pool.query(`SELECT id FROM v_b_contract_module_options WHERE module_key = $1`, [moduleKey]);
  if (exists.rows.length > 0) {
    moduleKey = `${moduleKey}_${Date.now().toString(36)}`;
  }
  const icon = String(payload.icon || "mdi:puzzle-outline").trim() || "mdi:puzzle-outline";
  const sortOrder = Number.isFinite(Number(payload.sortOrder)) ? Number(payload.sortOrder) : (await pool.query(`SELECT COALESCE(MAX(sort_order), 0) + 10 AS next FROM v_b_contract_module_options`)).rows[0].next;
  const result = await pool.query(`INSERT INTO v_b_contract_module_options (module_key, label, icon, enabled, sort_order)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id, module_key, label, icon, enabled, sort_order, created_at, updated_at`, [moduleKey, label, icon, payload.enabled !== false, sortOrder]);
...
export async function updateContractModuleOption(id, payload) {
  ...
  const result = await pool.query(`UPDATE v_b_contract_module_options
     SET label = $1,
         icon = $2,
         enabled = $3,
         sort_order = $4,
         updated_at = NOW()
     WHERE id = $5
     RETURNING id, module_key, label, icon, enabled, sort_order, created_at, updated_at`, [label, payload.icon !== undefined ? String(payload.icon).trim() || "mdi:puzzle-outline" : row.icon, payload.enabled !== undefined ? Boolean(payload.enabled) : row.enabled, payload.sortOrder !== undefined ? Number(payload.sortOrder) : row.sort_order, optionId]);
  return mapRow(result.rows[0]);
}
export async function deleteContractModuleOption(id) {
  await ensureContractModuleOptionsTable();
  const optionId = Number(id);
  const existing = await pool.query(`SELECT id, module_key FROM v_b_contract_module_options WHERE id = $1`, [optionId]);
  if (!existing.rows[0]) {
    const err = new Error("Option not found.");
    err.status = 404;
    throw err;
  ...
  if (usageCount > 0) {
    const err = new Error(usageCount === 1 ? "This option is used by 1 company and cannot be deleted." : `This option is used by ${usageCount} companies and cannot be deleted.`);
    err.status = 409;
    err.usageCount = usageCount;
    throw err;
  }
  const result = await pool.query(`DELETE FROM v_b_contract_module_options WHERE id = $1 RETURNING id`, [optionId]);
  if (!result.rows[0]) {
    const err = new Error("Option not found.");
    err.status = 404;
    throw err;
