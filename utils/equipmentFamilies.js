...
export async function listEquipmentFamilies({ includeDisabled = false, includeUsage = false } = {}) {
...
async function assertUniqueFamilyKey(familyKey, excludeId = null) {
  ...
  if (existing.rows.length) {
    const err = new Error("Cette clé de famille existe déjà.");
    err.status = 409;
    throw err;
  }
}
...
async function replaceFamilyFields(client, familyId, fields = []) {
  ...
  const normalized = (Array.isArray(fields) ? fields : [])
    .map((field, index) => {
      ...
      const fieldType = FIELD_TYPES.has(field.fieldType || field.field_type)
  ...
  for (const field of normalized) {
    await client.query(
      `INSERT INTO v_b_equipment_family_fields
         (family_id, field_key, label, field_type, required, display_order)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [familyId, field.fieldKey, field.label, field.fieldType, field.required, field.displayOrder]
    );
  }
...
export async function createEquipmentFamily(payload = {}) {
  ...
  if (!label) {
    const err = new Error("Le libellé de la famille est obligatoire.");
    err.status = 400;
    throw err;
  }
  ...
  try {
    ...
    const inserted = await client.query(
      `INSERT INTO v_b_equipment_family_definitions
      ...
      [
        familyKey,
        label,
        String(payload.icon || "mdi:devices").trim() || "mdi:devices",
        displayMode,
        payload.enabled !== false,
        Number.isFinite(Number(payload.sortOrder)) ? Number(payload.sortOrder) : 100,
        payload.honeycombQ == null || payload.honeycombQ === "" ? null : Number(payload.honeycombQ),
        payload.honeycombR == null || payload.honeycombR === "" ? null : Number(payload.honeycombR),
      ]
    );
...
export async function updateEquipmentFamily(id, payload = {}) {
  ...
  const existing = await pool.query(
    `SELECT * FROM v_b_equipment_family_definitions WHERE id = $1 LIMIT 1`,
    [id]
  );
  if (!existing.rows.length) {
    const err = new Error("Famille introuvable.");
    err.status = 404;
    throw err;
  }
  ...
  const nextFamilyKey = payload.familyKey !== undefined
    ? slugifyFamilyKey(payload.familyKey)
    : current.family_key;
  ...
  try {
    ...
    const updated = await client.query(
      `UPDATE v_b_equipment_family_definitions
       SET family_key = $2,
           label = $3,
      ...
      [
        ...
        nextFamilyKey,
        nextLabel,
        payload.icon !== undefined ? String(payload.icon || "mdi:devices").trim() : null,
        payload.displayMode !== undefined && DISPLAY_MODES.has(payload.displayMode)
          ? payload.displayMode
          : null,
        payload.enabled !== undefined ? Boolean(payload.enabled) : null,
        payload.sortOrder !== undefined ? Number(payload.sortOrder) : null,
        payload.honeycombQ !== undefined
          ? (payload.honeycombQ == null || payload.honeycombQ === "" ? null : Number(payload.honeycombQ))
          : current.honeycomb_q,
        payload.honeycombR !== undefined
          ? (payload.honeycombR == null || payload.honeycombR === "" ? null : Number(payload.honeycombR))
          : current.honeycomb_r,
      ]
    );
    ...
    if (nextFamilyKey !== current.family_key) {
      await client.query(
        `UPDATE v_b_clients_m_custom_equipment
         ...
         WHERE family_key = $1`,
        [current.family_key, nextFamilyKey]
      );
    }
...
export async function deleteEquipmentFamily(id) {
  await ensureEquipmentFamilyTables();
  const result = await pool.query(
    `DELETE FROM v_b_equipment_family_definitions WHERE id = $1 RETURNING family_key`,
    [id]
  );
  ...
  return { familyKey: result.rows[0].family_key };
}
...
export async function createClientCustomEquipment(clientId, familyKey, payload = {}) {
  ...
  if (!family || !family.enabled) {
    const err = new Error("Famille de matériel introuvable ou désactivée.");
    err.status = 404;
    throw err;
  }
  ...
  if (!name) {
    const err = new Error("Le nom du matériel est obligatoire.");
    err.status = 400;
    throw err;
  }

  const data = { ...(payload.fields && typeof payload.fields === "object" ? payload.fields : payload), name };
  delete data.name;
  const storedData = { name, ...data };

  const result = await pool.query(
    `INSERT INTO v_b_clients_m_custom_equipment
       (client_id, family_key, item_key, name, data, is_active)
     VALUES ($1, $2, $3, $4, $5, TRUE)
     RETURNING *`,
    [clientId, familyKey, name, name, JSON.stringify(storedData)]
  );
...
export async function updateClientCustomEquipment(clientId, familyKey, itemId, payload = {}) {
  ...
  const nextData = { ...current.data, ...incomingFields, name };
  delete nextData.id;
  ...
  const result = await pool.query(
    `UPDATE v_b_clients_m_custom_equipment
     ...
     WHERE id = $1 AND client_id = $2 AND family_key = $3
     RETURNING *`,
    [itemId, clientId, familyKey, name, JSON.stringify(nextData)]
  );
...
export async function deleteClientCustomEquipment(clientId, familyKey, itemId) {
  await ensureEquipmentFamilyTables();
  const result = await pool.query(
    `DELETE FROM v_b_clients_m_custom_equipment
     WHERE id = $1 AND client_id = $2 AND family_key = $3
     RETURNING id`,
    [itemId, clientId, familyKey]
  );
