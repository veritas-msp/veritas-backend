...
/** Equipment families scanned for automated monitoring. */
export const SUPERVISION_SCAN_FAMILIES = [
  { family: "servers", table: "v_b_clients_m_servers", hasAgent: true },
  { family: "stockage", table: "v_b_clients_m_stockage", hasAgent: false },
  { family: "firewall", table: "v_b_clients_m_firewall", hasAgent: false },
  { family: "switch", table: "v_b_clients_m_switch", hasAgent: false },
  { family: "wifi", table: "v_b_clients_m_wifi", hasAgent: false },
  { family: "routeur", table: "v_b_clients_m_routeur", hasAgent: false },
  { family: "internet", table: "v_b_clients_m_internet", hasAgent: false },
  { family: "toip", table: "v_b_clients_m_toip", hasAgent: false },
  { family: "alimentation", table: "v_b_clients_m_alimentation", hasAgent: false },
  { family: "ordinateurs", table: "v_b_clients_m_ordinateurs", hasAgent: true },
];
...
async function loadCheckmkMonitoringMap() {
  const result = await pool.query(
    `SELECT equipment_id::text, client_id, equipment_family, monitoring_data, last_synced_at
...
async function loadAgentLastSeenMap() {
  const result = await pool.query(
    `SELECT id::text AS agent_id, last_seen_at FROM v_b_rmm_agents WHERE status = 'active'`
  );
  return new Map(result.rows.map((r) => [r.agent_id, r.last_seen_at]));
}

/**
 * Loads the supervised inventory (all families for one client).
 */
export async function loadSupervisionEquipmentInventory({ clientId = null } = {}) {
  ...
  for (const spec of SUPERVISION_SCAN_FAMILIES) {
    ...
    const result = await pool.query(
      `SELECT id, client_id, name, data${agentSelect}${ipSelect}
       FROM ${spec.table}
       WHERE ${where}`,
      values
    );
    ...
    for (const row of result.rows) {
      ...
      items.push({
        ...
        equipmentName: row.name || data.nom || data.hostname || "Périphérique",
        data,
        ip: row.ip || null,
        agentId,
        lastSeenAt,
        checkmkSummary,
        isMkMapped,
        isMonitored: isEquipmentMonitoredInventoryItem({ agentId, data, isMkMapped }),
...
/**
 * Enables monitoring alerts on supervised equipment for a client (RMM or CheckMK).
 */
export async function enableMonitoringAlertsForClient(clientId, { equipmentFamilies = null } = {}) {
    ...
    await pool.query(
      `INSERT INTO v_b_equipment_monitoring_alerts
       ...
       ON CONFLICT (client_id, equipment_id, equipment_family) DO UPDATE SET
         ...
         alerts_enabled = true,
         updated_at = NOW()`,
      [clientId, item.equipmentId, item.equipmentFamily, item.equipmentName]
    );
    enabled += 1;
  }

  return { enabled, skipped, total: inventory.length };
}
  ...
  await pool.query(
    `INSERT INTO v_b_equipment_monitoring_alerts
     ...
     ON CONFLICT (client_id, equipment_id, equipment_family) DO UPDATE SET
       ...
       alerts_enabled = true,
       updated_at = NOW()`,
    [clientId, equipmentId, equipmentFamily, equipmentName || null]
  );
}

/**
 * Checks whether equipment is monitored (RMM agent or CheckMK mapping).
 */
export async function isEquipmentMonitoredInDb(clientId, equipmentId, equipmentFamily) {
  ...
  const mkResult = await pool.query(
    `SELECT 1 FROM v_b_equipment_checkmk_monitoring
     WHERE client_id = $1 AND equipment_id = $2::uuid AND equipment_family = $3
     LIMIT 1`,
    [clientId, equipmentId, family]
  );
  const isMkMapped = Boolean(
    mkResult.rows.length || data.checkmk_host_name || data.checkmkHostName
  );

  return isEquipmentMonitoredInventoryItem({ agentId, data, isMkMapped });
}
