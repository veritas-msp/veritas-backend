...
const CHECKMK_MAPPABLE_FAMILIES = new Set([
  "servers",
  "stockage",
  "firewall",
  "switch",
...
export function isEquipmentMonitoredInventoryItem({ agentId = null, data = {}, isMkMapped = false } = {}) {
...
function pushCriterion(criteria, key, detail = null) {
  criteria.push({ key, detail });
}

/**
 * Evaluates all active SUPERVISION_ALERT_CRITERIA for an equipment item.
 * @param {object} thresholds - thresholds from alert rules
 */
export function evaluateEquipmentSupervisionCriteria({
  ...
  const monitored = isEquipmentMonitoredInventoryItem({ agentId, data: d, isMkMapped });

  const t = {
    offlineAlertThresholdMinutes:
      thresholds?.offlineAlertThresholdMinutes ?? offlineAlertThresholdMinutes ?? 2880,
  ...
  if (family === "ordinateurs") {
    if (isRmmManaged(d, agentId)) {
      ...
      if (pendingUpdates >= t.updatesMinPending) {
        pushCriterion(criteria, "updates_pending", { pendingCount: pendingUpdates });
      }
  ...
  if (CHECKMK_MAPPABLE_FAMILIES.has(family)) {
    if (isMkMapped && monitored) {
      if (mkStatus === "critical") pushCriterion(criteria, "monitor_critical", checkmkSummary);
      else if (mkStatus === "warning") pushCriterion(criteria, "monitor_warning", checkmkSummary);
      else if (mkStatus === "no_data") pushCriterion(criteria, "no_data");
    } else if (!isMkMapped) {
      // Coverage alert only (not runtime alerts without monitoring)
      pushCriterion(criteria, "unmapped");
    }
  }

  // Inventory criteria (warranty, IP, etc.) only if equipment is monitored
  if (!monitored) {
    return criteria;
  }
  ...
  if (WARRANTY_FAMILIES.has(family)) {
    ...
    if (!skipWarranty) {
      ...
      if (warrantyStatus === "expired") pushCriterion(criteria, "warranty_expired", { date: warrantyDate });
      else if (warrantyStatus === "soon") pushCriterion(criteria, "warranty_soon", { date: warrantyDate });
    }
  }
  ...
  if (family === "firewall") {
    ...
    if (maintStatus === "expired") pushCriterion(criteria, "maintenance_expired", { date: maintDate });
    else if (maintStatus === "soon") pushCriterion(criteria, "maintenance_soon", { date: maintDate });
  }
  ...
  if (family === "alimentation") {
    ...
    if (batteryStatus === "expired") pushCriterion(criteria, "battery_expired", { date: batteryDate });
    else if (batteryStatus === "soon") pushCriterion(criteria, "battery_soon", { date: batteryDate });
  }
