...
export function normalizeNotificationLocale(locale, fallback = "fr") {
  const code = String(locale || fallback)
    .trim()
    .toLowerCase()
    .slice(0, 2);
  if (ALLOWED_LOCALES.includes(code)) return code;
  const safeFallback = ALLOWED_LOCALES.includes(fallback) ? fallback : "fr";
  return safeFallback;
}
...
export function getTestNotificationSample(type = "ticket_commented", locale = "fr") {
  const safeLocale = normalizeNotificationLocale(locale);
  const catalog = TEST_NOTIFICATION_SAMPLES[safeLocale] || TEST_NOTIFICATION_SAMPLES.fr;
