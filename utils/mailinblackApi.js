...
function buildAuthFailureMessage(lastStatus, lastBody, lastUrl = '') {
  if (lastStatus === 401) {
    return (
      "Clé API refusée. Vérifiez qu'elle est bien celle générée dans Espace manager → " +
      "Intégration → Clés API (lecture seule, produits Management et Protect activés)."
    );
  }
  if (lastStatus === 405) {
    return (
      "HTTP 405 — méthode refusée par Mailinblack. Utilisez uniquement la racine d'instance " +
      "(ex. https://app.mailinblack.com/mibc-fr-XX ou https://api.mailinblack.com), sans coller " +
      "le chemin /auth/api/... dans l'URL. La clé API va dans « Clé API », le client ID est optionnel " +
      "et rempli automatiquement après connexion."
    );
  }
  if (lastStatus) {
  ...
  return 'Connexion Mailinblack impossible — vérifiez l\'URL API et la clé API.';
}
...
export async function mailinblackAuthenticate(credentials = {}) {
  ...
  if (login?.trim() && password) {
    ...
    for (const authBase of authBases) {
      const { response, parsed } = await postMailinblackAuth(authBase, 'login', {
        login: login.trim(),
        password,
      });
  ...
  if (!key) {
    throw new Error(
      'Clé API Mailinblack requise — générez-la dans Espace manager → Intégration → Clés API.'
    );
  }
  ...
  const executeBodies = [
    ...
    ...(clientIdForAuth
      ? [
          { apiKey: key, clientId: clientIdForAuth },
          { key, clientId: clientIdForAuth },
          { authKey: key, clientId: clientIdForAuth },
          { clientId: clientIdForAuth, apiKey: key },
        ]
      : []),
  ];
  ...
  for (const authBase of authBases) {
    for (const executeBody of executeBodies) {
      try {
        const { response, parsed, url } = await postMailinblackAuth(
          authBase,
          'api-keys/execute',
          executeBody
        );
...
export async function mailinblackV2Request(
  apiUrl,
  session,
  module,
  path,
  ...
  throw new Error('Appel API Mailinblack impossible');
}

async function safeMailinblackCall(fn) {
  try {
    const data = await fn();
    return { ok: true, data, permissionDenied: false, error: null };
  } catch (err) {
    return {
      ok: false,
      data: null,
...
export async function mailinblackProtectRequest(apiUrl, apiKey, action, params = {}, method = 'POST') {
  ...
  try {
    return mailinblackV2Request(apiUrl, session, 'protect', path, { method, body: params });
  } catch {
...
export async function mailinblackProtectCheck(apiUrl, credentials) {
  const creds =
    typeof credentials === 'string'
      ? { apiUrl, authKey: credentials }
      : { apiUrl, ...credentials };
  const session = await mailinblackAuthenticate(creds);
  const probe = await safeMailinblackCall(() =>
    mailinblackV2Request(apiUrl, session, 'protect', 'senders', {
      method: 'GET',
      query: { page: 0, size: 1 },
    })
  );
  if (!probe.ok) {
    const domainsProbe = await safeMailinblackCall(() =>
      mailinblackV2Request(apiUrl, session, 'admin', 'domains', {
        method: 'GET',
        query: { page: 0, size: 1 },
      })
    );
    if (!domainsProbe.ok) {
      ...
      const raw = probe.error || domainsProbe.error || 'Connexion Mailinblack échouée';
      throw new Error(
        denied
          ? 'Accès refusé sur les APIs Protect/Admin. Vérifiez que la clé API a les droits Management et Protect (lecture seule suffit).'
          : raw
      );
    }
  }
...
function buildSectionFromResult(result, normalizer, columnsMeta = {}) {
  const { preItems, ...meta } = columnsMeta;
  const items =
    preItems ??
    (result.ok
      ? parseList(result.data)
          .map((item) => (normalizer ? normalizeListItem(item, normalizer) : item))
          .filter(Boolean)
      : []);
...
async function mailinblackFetchListSection(
  apiUrl,
  session,
  credentials,
  module,
  ...
  let lastResult = { ok: false, permissionDenied: false, error: 'Aucune donnée retournée' };

  for (const baseQuery of MAILINBLACK_LIST_QUERY_VARIANTS) {
    const query = { ...baseQuery };
    const result = await safeMailinblackCall(() =>
      mailinblackV2Request(apiUrl, session, module, path, {
        method: 'GET',
        query,
      })
    );
    ...
    let items = parseList(result.data)
      .map((item) => normalizeListItem(item, normalizer))
      .filter(Boolean);
    ...
    if (items.length > 0 && expectedTotal > items.length && baseQuery.page != null) {
      for (let page = 1; page < 10; page += 1) {
        const nextQuery = { ...baseQuery, page, pageNumber: page };
        const nextResult = await safeMailinblackCall(() =>
          mailinblackV2Request(apiUrl, session, module, path, {
            method: 'GET',
            query: nextQuery,
          })
        ...
        const nextItems = parseList(nextResult.data)
          .map((item) => normalizeListItem(item, normalizer))
          .filter(Boolean);
    ...
    if (items.length > 0) {
      return buildSectionFromResult(
        ...
        normalizer,
        { exploited, preItems: items }
      );
    }
  }

  return buildSectionFromResult(lastResult, normalizer, { exploited });
}
...
export async function mailinblackListCustomers(apiUrl, credentials) {
  ...
  const legacyAttempts = [
    () => mailinblackV2Request(apiUrl, session, 'protect', 'customers', { method: 'GET' }),
    () =>
...
export async function mailinblackGetCustomer(apiUrl, credentials, customerId) {
  ...
  const legacy = await safeMailinblackCall(() =>
    mailinblackProtectRequest(
      apiUrl,
      credentials.authKey || credentials.apiKey,
  ...
  const [domainsRes, usersRes] = await Promise.all([
    ...
    safeMailinblackCall(() =>
      mailinblackV2Request(apiUrl, session, 'admin', 'users', {
        method: 'GET',
        query: { page: 0, size: 200 },
      })
    ),
  ...
  const domains = domainsRes.ok
    ? parseList(domainsRes.data).map((item) => normalizeListItem(item, normalizeDomain)).filter(Boolean)
    : [];
  const users = usersRes.ok
    ? parseList(usersRes.data).map((item) => normalizeListItem(item, normalizeUser)).filter(Boolean)
    : [];
  ...
  return normalizeMailinblackCustomer(
    {
      ...
      name: credentials.label || 'Client Mailinblack',
...
export async function mailinblackBuildDashboard(apiUrl, credentials, customerId = null) {
  ...
  const [senders, spools, detectSpools, domains, users, customerRes] = await Promise.all([
    mailinblackFetchListSection(apiUrl, session, credentials, 'protect', 'senders', normalizeSender, {
      exploited: true,
    }),
    mailinblackFetchListSection(apiUrl, session, credentials, 'protect', 'spools', normalizeSpool, {
      exploited: true,
    }),
    mailinblackFetchListSection(
      apiUrl,
      session,
      credentials,
      'protect',
    ...
    effectiveCustomerId
      ? safeMailinblackCall(() =>
          mailinblackGetCustomer(apiUrl, credentials, effectiveCustomerId)
        )
      : Promise.resolve({ ok: false, data: null, permissionDenied: false, error: null }),
  ]);

  const customer =
    customerRes.ok && customerRes.data
      ? { status: 'ok', data: customerRes.data, error: null }
      : {
          status: effectiveCustomerId ? 'ok' : 'empty',
          data: effectiveCustomerId
            ? normalizeMailinblackCustomer(
                {
...
export function formatMailinblackSyncPayload(
  customer,
  mappingMode,
  mailinblackTenantId,
  extra = {}
  ...
  return {
    ...
    utilisateursProteges:
      usersSection?.total ??
      customer.usersCount ??
      (usersSection?.items?.length != null ? usersSection.items.length : 0),
    domainesSurveilles:
      domainsSection?.total ??
      customer.domainsCount ??
      (domainsSection?.items?.length != null ? domainsSection.items.length : 0),
