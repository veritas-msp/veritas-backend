...
import {
  REFERENCE_SCHEMA_BOOTSTRAP_SQL,
  REFERENCE_SCHEMA_MARKER,
  REFERENCE_SCHEMA_SEEDS_SQL,
  REFERENCE_SEEDS_MARKER,
  buildCreateTableSql,
  loadReferenceSchemaTables,
} from "./schemaFromCsv.js";

const INSTALL_STEPS = [{ kind: "seeds", name: REFERENCE_SEEDS_MARKER, label: "données initiales" }];
...
async function getAppliedTables(client) {
  const result = await client.query(
    "SELECT table_name FROM v_b_reference_schema_progress"
  );
...
export async function runNextPendingMigration() {
  ...
  try {
    ...
    if (!next) {
      await client.query(
        "INSERT INTO v_b_schema_migrations (filename) VALUES ($1) ON CONFLICT (filename) DO NOTHING",
        [REFERENCE_SCHEMA_MARKER]
      );
      return {
        done: true,
    ...
    try {
      ...
      await client.query(
        "INSERT INTO v_b_reference_schema_progress (table_name) VALUES ($1) ON CONFLICT (table_name) DO NOTHING",
        [next.name]
      );
      ...
      const target = next.kind === "table" ? `Table "${next.name}"` : "Données initiales";
...
export async function runPendingMigrations() {
  ...
  return { executed, total: progress.total };
}
