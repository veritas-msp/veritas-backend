...
export function equipmentDataSerial(data) {
  ...
  return normalizeSerial(
    parsed.numeroSerie ||
      parsed.serial ||
      parsed.serialNumber ||
      parsed.chassis?.serial ||
      parsed.chassis?.serialNumber
  );
}
...
export function equipmentDataMac(data) {
  const parsed = parseEquipmentData(data);
  return normalizeMac(
    parsed.mac || parsed.adresseMac || parsed.macAddress || parsed.network?.mac
  );
}
...
export function extractInventoryIdentifiers(inventory = {}, meta = {}) {
  ...
  const serial = normalizeSerial(
    chassis.serial ||
      ...
      inventory.numeroSerie ||
      inventory.serial ||
      meta.serial
  );
  const mac = normalizeMac(
    network.mac || inventory.mac || inventory.adresseMac || meta.mac
  );
...
export function findManualEquipmentMatch(rows, identifiers) {
  ...
  const { serial, mac, hostname } = identifiers;
  if (serial) {
    const bySerial = rows.filter((row) => equipmentDataSerial(row.data) === serial);
    if (bySerial.length === 1) return { row: bySerial[0], matchType: "serial" };
  }

  if (mac) {
    const byMac = rows.filter((row) => equipmentDataMac(row.data) === mac);
    if (byMac.length === 1) return { row: byMac[0], matchType: "mac" };
  }
  ...
  if (hostname) {
    const byName = rows.filter((row) => {
      ...
      const candidates = [
        ...
        String(data.nom || "").trim().toLowerCase(),
        String(data.netbios || "").trim().toLowerCase(),
        String(data.hostname || "").trim().toLowerCase(),
      ].filter(Boolean);
      return candidates.includes(hostname);
    });
    if (byName.length === 1) return { row: byName[0], matchType: "name" };
  }

  return null;
}

export async function findUnlinkedManualEquipment(pool, { clientId, table, inventory, meta }) {
  ...
  return match ? { ...match.row, matchType: match.matchType } : null;
}
