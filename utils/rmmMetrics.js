...
const METRIC_ID_TO_NAME = Object.fromEntries(
...
export function extractRmmMetricSamples(inventory = {}, collectors = {}) {
  ...
  if (collectors.hardware !== false) {
    ...
    if (Array.isArray(disks)) {
      for (const disk of disks) {
        ...
        samples.push({ metricId: RMM_METRIC_ID.DISK_USED_PCT, dimId, value: pct });
      }
    }
  }
  ...
  if (collectors.updates !== false) {
    ...
    const pending =
      ...
      (Array.isArray(updates.pending) ? updates.pending.length : null);
    if (pending != null) {
      const value = clampSmallInt(pending);
      if (value != null) {
        samples.push({ metricId: RMM_METRIC_ID.UPDATES_PENDING, dimId: 0, value });
      }
    }
  }
  ...
  if (collectors.performance !== false) {
    ...
    if (cpu != null) {
      samples.push({ metricId: RMM_METRIC_ID.CPU_USAGE_PCT, dimId: 0, value: cpu });
    }
    if (ram != null) {
      samples.push({ metricId: RMM_METRIC_ID.RAM_USAGE_PCT, dimId: 0, value: ram });
    }
  }
  ...
  if (collectors.sensors !== false) {
    ...
    if (temp != null && temp > 0) {
      samples.push({ metricId: RMM_METRIC_ID.CPU_TEMP_C, dimId: 0, value: temp });
    }
  }
...
export async function upsertRmmMetricSamples(agentId, samples, client = pool) {
  ...
  await client.query(
    `INSERT INTO v_b_rmm_metric_daily
     ...
     ON CONFLICT (agent_id, day_date, metric_id, dim_id) DO UPDATE SET
       ...
       sample_count = LEAST(32767, v_b_rmm_metric_daily.sample_count + 1)::smallint`,
    [agentId, metricIds, dimIds, values]
  );
...
export async function touchAgentMetricsSampleMeta(agentId, agentConfig = {}, samples = [], client = pool) {
  const patch = { metricsLastSampleAt: new Date().toISOString() };
  ...
  await client.query(
    `UPDATE v_b_rmm_agents
     ...
     WHERE id = $1`,
    [agentId, JSON.stringify(nextConfig)]
  );
}
...
export async function maybePurgeOldRmmMetrics(client = pool) {
  ...
  const defaultDays =
    resolveMetricsRuntimeConfig(global).retentionDays || RMM_METRICS_RETENTION_DAYS_DEFAULT;

  const result = await client.query(
    `DELETE FROM v_b_rmm_metric_daily d
...
export async function recordRmmMetricsFromHeartbeat(agent, inventory, settings = {}) {
  ...
  if (!samples.length) return { sampled: false };
  ...
  if (!shouldSampleNow(agentConfig, samples, metricsRuntime)) {
    return { sampled: false, skipped: "interval" };
  }
...
export async function fetchRmmMetricHistory(agentId, { metricId, dimId = 0, days = 90 } = {}) {
  const safeDays = Math.min(730, Math.max(1, Number.parseInt(String(days), 10) || 90));
  const result = await pool.query(
    `SELECT day_date, val_last, val_min, val_max, sample_count
    ...
    [agentId, metricId, dimId, safeDays]
  );
