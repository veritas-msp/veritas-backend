...
const RMM_SETTING_KEYS = ["RMM_HEARTBEAT_INTERVAL_MINUTES", "RMM_OFFLINE_THRESHOLD_MINUTES", "RMM_ALERTS_ENABLED_ON_ENROLL", "RMM_COLLECT_OS", "RMM_COLLECT_DOMAIN", "RMM_COLLECT_UPDATES", "RMM_COLLECT_LICENSE", "RMM_COLLECT_HARDWARE", "RMM_COLLECT_CHASSIS", "RMM_COLLECT_SESSION", "RMM_COLLECT_NETWORK", "RMM_COLLECT_SOFTWARE", "RMM_COLLECT_PRINTERS", "RMM_COLLECT_SHARES", "RMM_COLLECT_SERVICES", "RMM_COLLECT_PERIPHERALS", "RMM_COLLECT_PERFORMANCE", "RMM_COLLECT_SENSORS", "RMM_COLLECT_SECURITY", "RMM_METRICS_SAMPLE_INTERVAL_MINUTES", "RMM_METRICS_DISK_DELTA_PCT", "RMM_METRICS_RETENTION_DAYS"];
...
export async function fetchClientRmmOverrides(clientId) {
  ...
  const result = await pool.query(`SELECT overrides FROM v_b_rmm_client_settings WHERE client_id = $1 LIMIT 1`, [clientId]);
...
export async function listClientRmmSettings() {
  const result = await pool.query(`SELECT s.client_id, s.overrides, s.updated_at, c.name AS client_name
...
export async function saveClientRmmSettings(clientId, overrides = {}, updatedBy = null) {
  ...
  const result = await pool.query(`INSERT INTO v_b_rmm_client_settings (client_id, overrides, updated_by, updated_at)
     VALUES ($1, $2::jsonb, $3, NOW())
     ON CONFLICT (client_id) DO UPDATE SET
       overrides = EXCLUDED.overrides,
       updated_by = EXCLUDED.updated_by,
       updated_at = NOW()
     RETURNING overrides, updated_at`, [clientId, JSON.stringify(sanitized), updatedBy]);
