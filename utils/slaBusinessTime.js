...
export function buildDateInTimeZone(year, month, day, hour, minute, timeZone) {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    const diffMinutes = (year - parts.year) * 525600 + (month - parts.month) * 43200 + (day - parts.day) * 1440 + (hour - parts.hour) * 60 + (minute - parts.minute);
...
function findNextEnabledDayStart(parts, settings) {
  for (let offset = 1; offset <= 14; offset += 1) {
    ...
    const row = getScheduleRow(settings, getZonedDateParts(buildDateInTimeZone(next.year, next.month, next.day, 12, 0, settings.timezone), settings.timezone).dayOfWeek);
    if (row?.enabled) {
      const open = parseHHMM(row.open);
      return buildDateInTimeZone(next.year, next.month, next.day, Math.floor(open / 60), open % 60, settings.timezone);
    }
  }
