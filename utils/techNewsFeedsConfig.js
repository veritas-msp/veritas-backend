...
export const DEFAULT_FEEDS_BY_LOCALE = {
  fr: [
    { feed_key: "cert-fr", category: "security", source: "CERT-FR", url: "https://www.cert.ssi.gouv.fr/alerte/feed/" },
    { feed_key: "zdnet-fr", category: "news", source: "ZDNet France", url: "https://www.zdnet.fr/feeds/rss/" },
    {
    ...
    { feed_key: "it-connect", category: "tech", source: "IT-Connect", url: "https://www.it-connect.fr/feed/" },
    {
    ...
    { feed_key: "cisa", category: "cve", source: "CISA", url: "https://www.cisa.gov/cybersecurity-advisories/all.xml" },
    { feed_key: "exploitdb", category: "cve", source: "Exploit-DB", url: "https://www.exploit-db.com/rss.xml" },
  ],
  en: [
    { feed_key: "thn", category: "security", source: "The Hacker News", url: "https://feeds.feedburner.com/TheHackersNews" },
    { feed_key: "bleeping", category: "security", source: "BleepingComputer", url: "https://www.bleepingcomputer.com/feed/" },
    { feed_key: "cisa", category: "cve", source: "CISA", url: "https://www.cisa.gov/cybersecurity-advisories/all.xml" },
    { feed_key: "exploitdb", category: "cve", source: "Exploit-DB", url: "https://www.exploit-db.com/rss.xml" },
    { feed_key: "ars", category: "tech", source: "Ars Technica", url: "https://feeds.arstechnica.com/arstechnica/technology" },
    { feed_key: "register", category: "news", source: "The Register", url: "https://www.theregister.com/headlines.atom" },
  ],
  de: [
    { feed_key: "heise", category: "security", source: "Heise Security", url: "https://www.heise.de/security/rss/news.rss" },
    { feed_key: "golem", category: "tech", source: "Golem.de", url: "https://rss.golem.de/rss.php?tp=se&ty=1" },
    { feed_key: "cisa", category: "cve", source: "CISA", url: "https://www.cisa.gov/cybersecurity-advisories/all.xml" },
    { feed_key: "thn", category: "security", source: "The Hacker News", url: "https://feeds.feedburner.com/TheHackersNews" },
  ],
  it: [
    { feed_key: "punto", category: "tech", source: "Punto Informatico", url: "https://www.punto-informatico.it/feed/" },
    { feed_key: "hwupgrade", category: "news", source: "HWUpgrade", url: "https://www.hwupgrade.it/rss/rss_news.xml" },
    { feed_key: "cisa", category: "cve", source: "CISA", url: "https://www.cisa.gov/cybersecurity-advisories/all.xml" },
    { feed_key: "bleeping", category: "security", source: "BleepingComputer", url: "https://www.bleepingcomputer.com/feed/" },
  ],
  es: [
    { feed_key: "xataka", category: "tech", source: "Xataka", url: "https://www.xataka.com/index.xml" },
    { feed_key: "muycomputer", category: "news", source: "MuyComputer", url: "https://www.muycomputer.com/feed/" },
    { feed_key: "cisa", category: "cve", source: "CISA", url: "https://www.cisa.gov/cybersecurity-advisories/all.xml" },
    { feed_key: "thn", category: "security", source: "The Hacker News", url: "https://feeds.feedburner.com/TheHackersNews" },
  ],
};
...
async function seedDefaultsIfEmpty(locale) {
  ...
  const count = await pool.query(
    `SELECT COUNT(*)::int AS count FROM v_b_tech_news_feeds WHERE locale = $1`,
    [loc]
  );
  ...
  for (let i = 0; i < defaults.length; i += 1) {
    ...
    await pool.query(
      `INSERT INTO v_b_tech_news_feeds (locale, feed_key, source, url, category, enabled, sort_order)
       VALUES ($1, $2, $3, $4, $5, true, $6)
       ON CONFLICT (locale, feed_key) DO NOTHING`,
      [loc, feed.feed_key, feed.source, feed.url, feed.category, i * 10]
    );
  }
}

export async function listTechNewsFeeds(localeInput, { includeDisabled = true } = {}) {
  ...
  const result = await pool.query(
    `SELECT id, locale, feed_key, source, url, category, enabled, sort_order, created_at, updated_at
...
export async function getEnabledFeedsForLocale(localeInput) {
  const feeds = await listTechNewsFeeds(localeInput, { includeDisabled: false });
...
export async function createTechNewsFeed(payload) {
  ...
  if (!source || !url) {
    const err = new Error("Nom et URL du flux requis.");
    err.status = 400;
    throw err;
  }
  if (!/^https?:\/\//i.test(url)) {
    const err = new Error("L'URL du flux doit commencer par http:// ou https://");
    err.status = 400;
    throw err;
  }
  ...
  const exists = await pool.query(
    `SELECT id FROM v_b_tech_news_feeds WHERE locale = $1 AND feed_key = $2`,
    [locale, feedKey]
  );
  ...
  const sortOrder = Number.isFinite(Number(payload.sortOrder))
    ...
    : (await pool.query(`SELECT COALESCE(MAX(sort_order), 0) + 10 AS next FROM v_b_tech_news_feeds WHERE locale = $1`, [locale])).rows[0].next;

  const result = await pool.query(
    `INSERT INTO v_b_tech_news_feeds (locale, feed_key, source, url, category, enabled, sort_order)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING id, locale, feed_key, source, url, category, enabled, sort_order, created_at, updated_at`,
    [
      locale,
      feedKey,
      source,
...
export async function updateTechNewsFeed(id, payload) {
  ...
  if (!Number.isInteger(feedId) || feedId <= 0) {
    const err = new Error("Flux introuvable.");
    err.status = 404;
    throw err;
  }
  ...
  const result = await pool.query(
    ...
    [
      source,
      url,
      payload.category !== undefined ? normalizeCategory(payload.category) : row.category,
