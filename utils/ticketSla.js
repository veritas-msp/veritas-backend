...
function addHours(baseDate, hours, slaSettings) {
  ...
  return computeSlaDueAt({ startDate: baseDate, amount: hours, settingsInput: slaSettings });
}
...
export function buildSlaInfoForTicketSync({
  ...
  if (!hours || !slaSettings) return { enabled: false };

  const firstResponseDueAt = addHours(createdAt, hours.firstResponseHours, slaSettings);
  if (!firstResponseDueAt) return { enabled: false };
...
export function resolveTicketSlaInfo(
  ticket,
  { clientContrat, slaSettings } = {}
) {
  const resolvedContrat = clientContrat ?? ticket?.client_contrat ?? null;
  const existing = parseSlaInfo(ticket?.sla_info);
  const takeoverAt = ticket?.first_takeover_at
    || findFirstTakeoverAtFromHistory(ticket?.statusHistory)
    || existing.firstResponseAt
    || null;

  const refreshFirstResponse =
    isNewTicketStatus(ticket?.status) && !takeoverAt && !existing.firstResponseAt;
...
export async function persistTicketSlaInfoIfMissing(ticketId, slaInfo) {
  ...
  await pool.query(
    `UPDATE v_b_tickets
     ...
     WHERE id = $2
       AND COALESCE((sla_info->>'enabled')::boolean, false) = false`,
    [JSON.stringify(slaInfo), ticketId]
  );
}

export async function ensureTicketSlaInfoStored(ticketId) {
  const result = await pool.query(
    `SELECT t.id, t.priority, t.created_at, t.sla_info, t.client_id, c.contrat AS client_contrat
     FROM v_b_tickets t
...
export function enrichTicketWithSla(ticket, ctx = {}) {
  ...
  const { client_contrat: _clientContrat, first_public_comment_at: _firstCommentAt, ...publicTicket } =
    ticket;
  const ticketWithSla = { ...publicTicket, sla_info: resolvedSlaInfo };
...
export async function maybeRecordTakeoverSla(ticketId, oldStatus, newStatus) {
  ...
  const ticketResult = await pool.query(
    `SELECT t.sla_info, t.priority, t.created_at, c.contrat AS client_contrat
     FROM v_b_tickets t
     LEFT JOIN v_b_clients c ON c.id = t.client_id
     WHERE t.id = $1`,
    [ticketId]
  ...
  const takeoverAt =
    findFirstTakeoverAtFromHistory(historyResult.rows) || sla.firstResponseAt || null;
  if (!takeoverAt) return;

  const slaSettings = await loadSlaSettings();
  const nextSla = applyTakeoverToSlaInfo(sla, takeoverAt, slaSettings);

  await pool.query("UPDATE v_b_tickets SET sla_info = $1::jsonb, updated_at = NOW() WHERE id = $2", [
    JSON.stringify(nextSla),
    ticketId,
  ]);
}
