...
export const PRESET_AVATAR_IDS = ["blue", "green", "orange", "purple", "rose", "slate", "teal", "amber"];
...
export async function upsertUserAvatarSetting(userId, avatar) {
  if (!userId) return;
  if (!avatar) {
    await pool.query(`DELETE FROM v_b_users_settings WHERE user_id = $1 AND setting_key = $2`, [userId, USER_AVATAR_SETTING_KEY]);
    return;
  }
...
export async function loadAvatarsByUserIds(userIds = []) {
  ...
  const {
    rows
  } = await pool.query(`SELECT u.id, av.setting_value AS avatar_setting_raw
     FROM v_b_users u
     LEFT JOIN v_b_users_settings av
       ON av.user_id = u.id AND av.setting_key = $2
     WHERE u.id::text = ANY($1::text[])`, [unique, USER_AVATAR_SETTING_KEY]);
  rows.forEach(row => {
...
export async function loadAuthorProfilesByUserIds(userIds = []) {
  ...
  const {
    rows
  } = await pool.query(`SELECT u.id, u.username, u.email, u.role,
     ...
     WHERE u.id::text = ANY($1::text[])`, [unique, TICKET_HELPDESK_DISPLAY_NAME_KEY, USER_AVATAR_SETTING_KEY]);
  rows.forEach(row => {
    const helpdeskRaw = parseJsonValue(row.helpdesk_name_raw);
    const helpdeskName = typeof helpdeskRaw === "string" ? helpdeskRaw.trim() : helpdeskRaw != null ? String(helpdeskRaw).trim() : "";
    const isClient = String(row.role || "").toLowerCase() === "client";
    const displayName = isClient ? row.username || row.email || "Client" : helpdeskName || row.username || row.email || "Agent";
    const avatar = parseUserAvatarSetting(row.avatar_setting_raw);
