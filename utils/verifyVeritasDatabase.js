...
export const VERITAS_CORE_TABLES = [
  { key: "users", table: "v_b_users" },
  { key: "settings", table: "v_b_settings" },
  { key: "clients", table: "v_b_clients" },
  { key: "tickets", table: "v_b_tickets" },
  { key: "migrations", table: "v_b_schema_migrations" },
];
...
export async function verifyVeritasConformance(runQuery) {
  ...
  for (const item of VERITAS_CORE_TABLES) {
    ...
    checks.push({ key: item.key, table: item.table, ok });
  ...
  if (!missingTables.includes("v_b_schema_migrations")) {
    try {
      const markerResult = await runQuery(
